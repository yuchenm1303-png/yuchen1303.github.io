...
const WORKER_VERSION = "2026-05-15-gemini-provider-1";

const TOOL_REGISTRY = [
  { name: "ledger.draft_records", action: "draft", title: "整理待确认账单", description: "当用户表达真实收支时，把内容整理成待确认账单；缺金额等关键信息时先追问。" },
  { name: "mobile.set_alarm", action: "mobile_command", commandType: "set_alarm", title: "设置系统闹钟", description: "当用户要求叫醒、闹钟、提醒某个具体时间时使用。" },
  { name: "mobile.open_app", action: "mobile_command", commandType: "open_app", title: "打开手机应用", description: "当用户要求打开微信、支付宝、淘宝、QQ、百度地图等应用时使用。" },
  { name: "mobile.navigate", action: "mobile_command", commandType: "navigate", title: "地图导航", description: "当用户要求导航、回家、去某地怎么走、带我去某地时使用。" },
];
  ...
  async fetch(request, env) {
    ...
    if (request.method === "GET" && url.pathname === "/health") {
      ...
      return json({
        ...
        provider,
        mode: "gemini_first_with_workers_ai_fallback",
    ...
    if (conversation.length > 5000) return json({ error: "conversation is too long", code: "conversation_too_long", version: WORKER_VERSION }, 400, corsHeaders);
    ...
    if (env.GEMINI_API_KEY) {
      provider = "gemini";
      try {
        raw = await runGemini({ env, instructions, context });
      } catch (error) {
    ...
    if (!raw && env.AI) {
      provider = "workers_ai";
      try {
        raw = await runWorkersAI({ env, instructions, context, schema });
      } catch (error) {
        providerError = String(error?.message || error);
      }
    }
    ...
    if (!raw) {
      return json({
        reply: providerError
          ? `云端 AI 暂时不可用：${providerError.slice(0, 160)}`
          : "云端 AI 还没有配置成功。我现在只能处理记账、闹钟、打开应用和导航等本地规则任务。",
        action: "chat",
        records: [],
        mobileCommand: null,
    ...
    return json({
      ...
      records,
      mobileCommand,
      source: provider === "gemini" ? "gemini_ai" : "workers_ai",
      provider,
      version: WORKER_VERSION,
    }, 200, corsHeaders);
  },
...
async function runWorkersAI({ env, instructions, context, schema }) {
...
function json(payload, status = 200, corsHeaders = {}) {
  return new Response(JSON.stringify(payload), { status, headers: { ...jsonHeaders, ...corsHeaders } });
}
...
function buildInstructions(now) {
  return [
    ...
    "输出协议必须返回 JSON：{ reply, action, records, mobileCommand }。不要输出 Markdown，不要用代码块。",
    "action=chat 时 records 必须是空数组，mobileCommand 可以为空。",
    "action=draft 时 records 返回待确认账单。",
    "action=mobile_command 时 records 必须是空数组，mobileCommand 必须完整。",
...
function tryDeterministicReply({ lastUserText, pendingDraft, ledgerContext, now }) {
  ...
  if (/^(你好|您好|嗨|哈喽|在吗|hello|hi)$/iu.test(text)) return { reply: "我在。你可以直接说任务，比如记账、设置闹钟、打开应用、导航，也可以正常聊天。", action: "chat", records: [], mobileCommand: null };
  if (/(你有|你会|有哪些).*(功能|能做什么|会干什么)/u.test(text)) return { reply: "我可以聊天、记账、查账本，也能生成设置闹钟、打开应用、地图导航这些手机动作卡片。", action: "chat", records: [], mobileCommand: null };
  const mobileCommand = parseMobileCommand(text, now);
  if (mobileCommand) return { reply: createMobileReply(mobileCommand), action: "mobile_command", records: [], mobileCommand };
  const simpleRecords = parseSimpleRecords(text, now);
  if (simpleRecords.length) return { reply: `我先整理出 ${simpleRecords.length} 笔待确认账单，你回复“好”我就帮你保存。`, action: "draft", records: simpleRecords, mobileCommand: null };
  const requestedItem = extractIncompleteItem(text) || extractStandaloneItem(text);
  if (requestedItem) return { reply: `${requestedItem}花了多少钱？`, action: "chat", records: [], mobileCommand: null };
...
function parseAlarmCommand(text, now) {
  ...
  const label = rawLabel.replace(/(明天|后天|今天|今晚|明早|明晚|上午|下午|晚上|早上|凌晨|中午)/gu, "").replace(/\d{1,2}(?:[:：点时]\d{0,2})?分?/gu, "").replace(/^(去|要|一下|起床)/u, "").trim() || (/起床|叫醒/u.test(text) ? "起床" : "提醒");
  return { type: "set_alarm", title: "设置闹钟", summary: `${date} ${String(hour).padStart(2, "0")}:${String(minute).padStart(2, "0")}`, params: { date, hour, minute, label } };
}
...
function parseNavigationCommand(text) {
  ...
  const destinationMatch = text.match(/(?:导航(?:到|去)?|路线到|带我去|怎么去|怎么到)\s*([\u4e00-\u9fa5A-Za-z0-9·.\- ]+)$/u) || text.match(/去\s*([\u4e00-\u9fa5A-Za-z0-9·.\- ]+?)(?:怎么走|路线|导航)$/u);
  let destination = destinationMatch?.[1]?.trim() || "";
  if (/回家|到家|去家|家里|我家|我想回家/u.test(text)) destination = "家";
  destination = destination.replace(/^(百度地图|高德地图|地图|帮我|请|给我)/u, "").replace(/(?:怎么走|路线|导航)$/u, "").trim();
...
function parseSimpleRecords(text, now) {
  ...
  const records = parts.map((part) => {
    ...
    return { title: cleanTitle(part), amount, type: inferType(part), category: inferCategory(part), date: /昨天/u.test(part) ? shiftDate(now, -1) : /前天/u.test(part) ? shiftDate(now, -2) : now };
  }).filter(Boolean);
...
function extractStandaloneItem(text) { if (/\d/u.test(text)) return null; return /^(早餐|午饭|午餐|晚饭|晚餐|奶茶|咖啡|地铁|公交|打车|火锅|外卖)$/u.test(text) ? normalizeItem(text) : null; }
function normalizeItem(text) { return String(text || "").replace(/^(一笔|一个|一下)/u, "").replace(/金额$/u, "").trim(); }
function extractCategoryQuery(text) { return text.match(/这个月(餐饮|交通|购物|居住|饮品|工资|礼物|其他)(?:花了多少|支出多少|多少钱)/u)?.[1] || null; }
function sumCategoryThisMonth(records, category, now) { const prefix = String(now).slice(0, 7); return (Array.isArray(records) ? records : []).filter((record) => record.type === "expense" && record.category === category && String(record.date).startsWith(prefix)).reduce((sum, record) => sum + Number(record.amount || 0), 0); }
...
function shiftDate(isoDate, offsetDays) { const date = new Date(`${isoDate}T00:00:00Z`); date.setUTCDate(date.getUTCDate() + offsetDays); return date.toISOString().slice(0, 10); }
...
function extractWorkersAiText(result) {
  ...
  if (Array.isArray(result?.content)) return result.content.map((item) => item?.text || item).join("\n");
...
function parseAiJson(raw) {
  ...
  if (!text) return null;
  const direct = tryJson(text); if (direct) return direct;
  const unfenced = text.replace(/^```(?:json)?\s*/i, "").replace(/```$/i, "").trim();
  const fromFence = tryJson(unfenced); if (fromFence) return fromFence;
  const balanced = extractFirstBalancedObject(unfenced);
  return balanced ? tryJson(balanced) : null;
}
...
function cleanRawAiText(raw) { const text = String(raw || "").replace(/^```(?:json)?\s*/i, "").replace(/```$/i, "").trim(); if (!text || (text.startsWith("{") && text.endsWith("}"))) return ""; return text.slice(0, 700); }
...
function normalizeMessages(messages) { if (!Array.isArray(messages)) return []; return messages.slice(-10).map((message) => ({ role: message?.role === "assistant" ? "assistant" : "user", content: String(message?.content || "").trim().slice(0, 500) })).filter((message) => message.content); }
function buildConversation(messages) { return messages.map((message) => `${message.role === "assistant" ? "助手" : "用户"}：${message.content}`).join("\n"); }
function sanitizeRecords(records, fallbackDate) { if (!Array.isArray(records)) return []; return records.slice(0, 10).map((record) => ({ title: String(record?.title || "未命名账单").trim().slice(0, 30) || "未命名账单", amount: Number(record?.amount), type: record?.type === "income" ? "income" : "expense", category: ALLOWED_CATEGORIES.includes(record?.category) ? record.category : "其他", date: normalizeIsoDate(record?.date) || fallbackDate })).filter((record) => Number.isFinite(record.amount) && record.amount > 0); }
...
function sanitizeClientTools(tools) { if (!Array.isArray(tools)) return []; return tools.slice(0, 12).map((tool) => ({ name: String(tool?.name || "").trim().slice(0, 50), action: String(tool?.action || "").trim().slice(0, 40), commandType: String(tool?.commandType || "").trim().slice(0, 40), title: String(tool?.title || "").trim().slice(0, 40) })).filter((tool) => tool.name && tool.action); }
...
function sanitizeMobileCommand(command, fallbackDate) {
  ...
  if (type === "set_alarm") {
    const hour = Number(params.hour); const minute = Number(params.minute || 0);
...
function sanitizeAction(action, records, pendingDraft, mobileCommand) {
  if (!ALLOWED_ACTIONS.includes(action)) { if (mobileCommand) return "mobile_command"; return records.length ? "draft" : "chat"; }
