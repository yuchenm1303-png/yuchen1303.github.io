  ...
  async fetch(request, env) {
    ...
    if (request.method === "GET" && url.pathname === "/health") {
      return json({
        ...
        tools: ["weather", "wikipedia", "webpage", "calculator", "datetime", "mobile_actions", "ledger", "optional_search"],
      }, 200, corsHeaders);
    }
    ...
    const now = normalizeDate(body?.now) || todayCN();
    const messages = normalizeMessages(body?.messages);
    const text = String(body?.text || lastUser(messages) || "").trim();
    const pendingDraft = records(body?.pendingDraft, now);
    const ledgerContext = safeLedger(body?.ledgerContext, now);
    ...
    const online = await onlineTool(text, env, now);
    if (online) return json({ ...online, version: VERSION }, 200, corsHeaders);

    const rule = ruleReply(text, pendingDraft, ledgerContext, now);
    ...
    if (env.GEMINI_API_KEY) {
      provider = "gemini";
      try { raw = await gemini(env, prompt); }
      catch (e) { err = String(e?.message || e); }
    }
    if (!raw && env.AI) {
      provider = "workers_ai";
      try { raw = await workersAI(env, prompt); }
      catch (e) { err = String(e?.message || e); }
    }

    if (!raw) {
      return json({
        reply: err ? `云端 AI 暂时不可用：${err.slice(0, 160)}` : "云端 AI 还没配置成功，但我仍可以查天气、百科、读网页、计算、记账、设置闹钟、打开应用和导航。",
        action: "chat",
        records: [],
        mobileCommand: null,
    ...
    if (!parsed) {
      return json({ reply: cleanText(raw) || "我可以继续聊。", action: "chat", records: [], mobileCommand: null, source: provider === "gemini" ? "gemini_text_fallback" : "workers_ai_text_fallback", version: VERSION }, 200, corsHeaders);
    }

    const outRecords = records(parsed.records, now);
    const mobileCommand = mobile(parsed.mobileCommand, now);
    const action = actionName(parsed.action, outRecords, pendingDraft, mobileCommand);
    const reply = String(parsed.reply || "").trim().slice(0, 900) || fallbackReply(action, outRecords, mobileCommand);
    return json({ reply, action, records: outRecords, mobileCommand, source: provider === "gemini" ? "gemini_ai" : "workers_ai", provider, version: VERSION }, 200, corsHeaders);
  },
};

async function onlineTool(text, env, now) {
  const value = String(text || "").trim();
  if (!value) return null;
  const url = value.match(/https?:\/\/[^\s]+/i)?.[0] || "";
  if (url && /(总结|读取|看看|分析|网页|链接|这个)/u.test(value)) return webpage(url);
...
async function wiki(topic) {
  try {
    const s = await fetchJson(`https://zh.wikipedia.org/w/rest.php/v1/search/page?q=${encodeURIComponent(topic)}&limit=1`);
    const key = s?.pages?.[0]?.key;
    if (!key) return pack(`没有找到“${topic}”的百科摘要。`, "wiki_tool");
    const sum = await fetchJson(`https://zh.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(key)}`);
    return pack(sum?.extract ? `${sum.title || topic}：${String(sum.extract).slice(0, 520)}` : `找到“${topic}”，但没有可用摘要。`, "wiki_tool");
  } catch (e) { return pack(`百科查询失败：${String(e?.message || e).slice(0, 120)}`, "wiki_tool"); }
}

async function webpage(url) {
  try {
    const res = await fetch(url, { headers: { "user-agent": "AI-Assistant-Worker/1.0" } });
  ...
  } catch (e) { return pack(`网页读取失败：${String(e?.message || e).slice(0, 120)}`, "webpage_tool"); }
}

async function search(text, env) {
  ...
  try {
    ...
    if (env.BRAVE_SEARCH_API_KEY) {
      const data = await fetchJson(`https://api.search.brave.com/res/v1/web/search?q=${encodeURIComponent(query)}&count=5`, { headers: { accept: "application/json", "x-subscription-token": env.BRAVE_SEARCH_API_KEY } });
      const rows = (data?.web?.results || []).slice(0, 5).map((x, i) => `${i + 1}. ${x.title || "结果"}：${x.description || x.url || ""}`).join("\n");
      return pack(`搜索结果：\n${rows || "没有找到结果。"}`, "web_search_tool");
    }
    if (env.SERPER_API_KEY) {
      const data = await fetchJson("https://google.serper.dev/search", { method: "POST", headers: { "content-type": "application/json", "X-API-KEY": env.SERPER_API_KEY }, body: JSON.stringify({ q: query, num: 5 }) });
      const rows = (data?.organic || []).slice(0, 5).map((x, i) => `${i + 1}. ${x.title || "结果"}：${x.snippet || x.link || ""}`).join("\n");
      return pack(`搜索结果：\n${rows || "没有找到结果。"}`, "web_search_tool");
    }
  ...
  } catch (e) { return pack(`联网搜索失败：${String(e?.message || e).slice(0, 150)}`, "web_search_tool"); }
}
...
async function gemini(env, prompt) {
  ...
  const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent?key=${encodeURIComponent(env.GEMINI_API_KEY)}`, {
    ...
    body: JSON.stringify({ systemInstruction: { parts: [{ text: prompt.system }] }, contents: [{ role: "user", parts: [{ text: prompt.user }] }], generationConfig: { temperature: 0.18, maxOutputTokens: 900, responseMimeType: "application/json" } }),
  });
...
async function workersAI(env, prompt) {
  const result = await env.AI.run(env.AI_MODEL || "@cf/meta/llama-3.1-8b-instruct", { messages: [{ role: "system", content: prompt.system }, { role: "user", content: prompt.user }], temperature: 0.15, max_tokens: 700, response_format: { type: "json_schema", json_schema: schema() } });
...
function makePrompt({ now, pendingDraft, ledgerContext, conversation, env }) {
  return {
    system: ["你是中文手机 AI 助手。普通聊天正常回答；记账、闹钟、打开应用、导航要整理成 JSON。", "输出必须是 JSON：{ reply, action, records, mobileCommand }，不要 Markdown。", "action 只能是 chat、draft、confirm_pending、cancel_pending、mobile_command。", `今天日期：${now}。`].join("\n"),
...
function ruleReply(text, pendingDraft, ledgerContext, now) {
...
function simpleRecords(text, now) { if (/(我付了|垫付|平摊|AA)/u.test(text)) return []; const parts = text.split(/[，,。；;、\n]/).map(x => x.trim()).filter(Boolean); const rs = parts.map(p => { const m = p.match(/(\d+(?:\.\d+)?)/u); if (!m) return null; const amount = Number(m[1]); if (!Number.isFinite(amount) || amount <= 0) return null; return { title: p.replace(/[0-9.元块钱]/gu, "").trim() || "未命名账单", amount, type: /(收入|工资|兼职|奖金|报销|收到|进账)/u.test(p) ? "income" : "expense", category: category(p), date: /昨天/u.test(p) ? shiftDate(now, -1) : now }; }).filter(Boolean); return rs.length === parts.length ? rs : []; }
function category(t) { if (/(饭|早餐|午餐|晚餐|外卖|餐)/u.test(t)) return "餐饮"; if (/(奶茶|咖啡|饮料|茶)/u.test(t)) return "饮品"; if (/(打车|公交|地铁|高铁|火车|机票)/u.test(t)) return "交通"; if (/(买|淘宝|京东|购物|超市)/u.test(t)) return "购物"; if (/(房租|水电|物业)/u.test(t)) return "居住"; if (/(工资|兼职|奖金|补贴|报销)/u.test(t)) return "工资"; return "其他"; }
function pack(reply, source) { return { reply, action: "chat", records: [], mobileCommand: null, source }; }
function json(payload, status = 200, headers = {}) { return new Response(JSON.stringify(payload), { status, headers: { ...jsonHeaders, ...headers } }); }
...
function parseJson(raw) { const text = String(raw || "").trim(); try { return JSON.parse(text); } catch {} const unfenced = text.replace(/^```(?:json)?\s*/i, "").replace(/```$/i, "").trim(); try { return JSON.parse(unfenced); } catch {} const obj = balanced(unfenced); if (!obj) return null; try { return JSON.parse(obj); } catch { return null; } }
function balanced(text) { const start = text.indexOf("{"); if (start < 0) return null; let depth = 0, str = false, esc = false; for (let i = start; i < text.length; i++) { const ch = text[i]; if (str) { if (esc) esc = false; else if (ch === "\\") esc = true; else if (ch === '"') str = false; continue; } if (ch === '"') str = true; else if (ch === "{") depth++; else if (ch === "}") { depth--; if (depth === 0) return text.slice(start, i + 1); } } return null; }
function schema() { return { type: "object", properties: { reply: { type: "string" }, action: { type: "string" }, records: { type: "array" }, mobileCommand: { type: "object" } }, required: ["reply", "action", "records"] }; }
function records(list, date) { if (!Array.isArray(list)) return []; return list.slice(0, 10).map(r => ({ title: String(r?.title || "未命名账单").slice(0, 30), amount: Number(r?.amount), type: r?.type === "income" ? "income" : "expense", category: CATEGORIES.includes(r?.category) ? r.category : "其他", date: normalizeDate(r?.date) || date })).filter(r => Number.isFinite(r.amount) && r.amount > 0); }
function mobile(cmd, date) { if (!cmd || typeof cmd !== "object") return null; if (["set_alarm", "open_app", "navigate"].includes(cmd.type)) return cmd; return null; }
function actionName(action, rs, pending, cmd) { if (!ACTIONS.includes(action)) return cmd ? "mobile_command" : rs.length ? "draft" : "chat"; if (action === "draft" && !rs.length) return "chat"; if (action === "mobile_command" && !cmd) return "chat"; if ((action === "confirm_pending" || action === "cancel_pending") && !pending.length) return "chat"; return action; }
...
function safeLedger(x, date) { const s = x && typeof x === "object" ? x : {}; return { today: date, summary: s.summary || {}, recentRecords: records(s.recentRecords, date).slice(0, 60) }; }
...
function normalizeDate(v) { const t = String(v || ""); return /^\d{4}-\d{2}-\d{2}$/.test(t) ? t : null; }
...
function r(v) { return Number.isFinite(Number(v)) ? Math.round(Number(v)) : "未知"; }
...
function cleanText(raw) { const text = String(raw || "").replace(/^```(?:json)?\s*/i, "").replace(/```$/i, "").trim(); if (!text || (text.startsWith("{") && text.endsWith("}"))) return ""; return text.slice(0, 900); }
