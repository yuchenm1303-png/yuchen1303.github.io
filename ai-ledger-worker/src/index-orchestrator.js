...
const ORCHESTRATOR_VERSION = "ai-ledger-orchestrator-v7-model-picker-search-citations";
  ...
  async fetch(request, env, ctx) {
    ...
    if (request.method === "GET" && url.pathname === "/health") {
      ...
      return json({
        ...
        mode: "cloud_brain_local_executor_provider_pool_model_picker_search_citations",
        ...
        providerPool: providerPool(env).map((item) => ({ provider: item.provider, model: item.model, label: item.label, id: item.id, tasks: item.tasks })),
    ...
    if (hasAttachments) return attachmentGateway.fetch(cloneRequestWithJson(request, { ...body, memoryContext: memory, modelPreference }), env, ctx);
    ...
    if (intent.intent === "weather.query" && !isWeatherLikeText(text) && !isWeatherLocationFollowup(body, text)) intent = { intent: "chat", confidence: 0.72, toolInput: {}, reason: "weather_false_positive_guard", provider: intent.provider, model: intent.model, modelLabel: intent.modelLabel };
    ...
    if (intent.intent === "chat") return json(await chatWithProviderPool(env, body, text, memory, modelPreference), 200, corsHeaders);

    return delegateCommandWorker(request, env, ctx, { ...body, memoryContext: memory, modelPreference, orchestrator: { version: ORCHESTRATOR_VERSION, intent: intent.intent, confidence: intent.confidence, toolInput: intent.toolInput || {}, plannerProvider: intent.provider, plannerModel: intent.model, instruction: "For actionable local tasks, return structured command JSON. For normal chat, return action=chat." } }, intent);
  },
};
...
async function safeHealth(worker, originalRequest, env, ctx) { try { const url = new URL(originalRequest.url); const res = await worker.fetch(new Request(`${url.origin}/health`, { method: "GET", headers: originalRequest.headers }), env, ctx); return await res.json().catch(() => ({ ok: res.ok, status: res.status })); } catch (error) { return { ok: false, error: String(error?.message || error).slice(0, 120) }; } }
function cloneRequestWithJson(originalRequest, body) { const headers = new Headers(originalRequest.headers); headers.set("content-type", "application/json; charset=utf-8"); headers.delete("content-length"); return new Request(originalRequest.url, { method: "POST", headers, body: JSON.stringify(body) }); }
...
async function classifyIntent(env, body, text, memory = {}) {
  ...
  if (provider === "nvidia" && env.NVIDIA_API_KEY) { const nvidia = await classifyWithNvidia(env, body, text, memory, fallback); if (nvidia) return nvidia; }
  if (env.GEMINI_API_KEY) { const gemini = await classifyWithGemini(env, body, text, memory, fallback); if (gemini) return gemini; }
  if (env.NVIDIA_API_KEY && provider !== "nvidia") { const nvidia = await classifyWithNvidia(env, body, text, memory, fallback); if (nvidia) return nvidia; }
  return fallback;
}
function buildPlannerPrompt(body, text, memory) { const messages = Array.isArray(body?.messages) ? body.messages.slice(-10) : []; return ["你是 AI 助手的意图规划器。你只判断最后一句用户话应该交给哪一个工具或对话层，不回答正文。", "必须只返回 JSON object，不要 markdown，不要解释。", "允许 intent：attachment.analyze, weather.query, web.search, news.query, navigation.start, navigation.modify, navigation.preference.set, alarm.set, app.open, ledger.create, chat。", "返回格式必须是：", JSON.stringify({ intent: "chat", confidence: 0.8, toolInput: {}, reason: "short" }), "硬规则：", "1. 搜一下/搜索/查一下/联网查/上网查，且没有天气词 → web.search 或 news.query，不能判成 weather.query。", "2. 新闻、大新闻、热点、今日新闻、最新消息 → news.query。不要把“新闻”当作城市。", "3. 天气、气温、下雨、穿衣建议、几度、带伞 → weather.query。toolInput.location 有城市就填，没有则空。", "4. 如果上一条助手问城市天气，用户只回城市名，才判为 weather.query。", "5. 导航、修改导航、保存家/学校/公司/宿舍地址、默认地图/默认出行方式 → 对应 navigation intent。", "6. 设置提醒/闹钟 → alarm.set。打开 App → app.open。真实收支 → ledger.create。", "7. 普通解释、定义、学习问题 → chat。", "本地记忆：", JSON.stringify(memory || {}), "对话历史：", JSON.stringify(messages), "最后一句：", text].join("\n"); }
async function classifyWithNvidia(env, body, text, memory, fallback) { try { const model = nvidiaPlannerModel(env); const raw = await callNvidiaChat(env, model, [{ role: "system", content: "You are a strict JSON-only intent planner. Return valid JSON only." }, { role: "user", content: buildPlannerPrompt(body, text, memory) }], 320, 0); const parsed = parseJsonObject(raw); if (!parsed) return null; return normalizeIntent(parsed, fallback, modelMeta("NVIDIA NIM", model, nvidiaModelLabel(model))); } catch { return null; } }
async function classifyWithGemini(env, body, text, memory, fallback) { try { const model = geminiModel(env); const raw = await callGeminiText(env, model, buildPlannerPrompt(body, text, memory), 320, 0, true); const parsed = parseJsonObject(raw); if (!parsed) return null; return normalizeIntent(parsed, fallback, modelMeta("Gemini", model, `${geminiModelLabel(model)} Planner`)); } catch { return null; } }
function parseJsonObject(raw) { const text = String(raw || "").trim().replace(/^```json\s*/i, "").replace(/^```\s*/i, "").replace(/```$/i, "").trim(); try { return JSON.parse(text); } catch {} const match = text.match(/\{[\s\S]*\}/); if (!match) return null; try { return JSON.parse(match[0]); } catch { return null; } }
function normalizeIntent(input, fallback, meta = {}) { const intent = ALLOWED_INTENTS.has(input?.intent) ? input.intent : fallback.intent; const confidence = Number(input?.confidence); return { intent, confidence: Number.isFinite(confidence) ? Math.max(0, Math.min(1, confidence)) : fallback.confidence, toolInput: input?.toolInput && typeof input.toolInput === "object" ? input.toolInput : fallback.toolInput || {}, reason: String(input?.reason || fallback.reason || "").slice(0, 120), ...meta }; }

function heuristicIntent(body, text) { const raw = String(text || "").trim(); const clean = raw.replace(/[？?。！!\s]/g, ""); const prevAssistant = previousAssistantText(body); if (isExplicitSearchQuery(raw) && !isWeatherLikeText(raw)) return { intent: /新闻|热点|消息/u.test(raw) ? "news.query" : "web.search", confidence: 0.9, toolInput: { query: cleanSearchQuery(raw) }, reason: "explicit search", ...modelMeta("Orchestrator", "heuristic", "Local Heuristic") }; if (isWeatherLikeText(raw)) return { intent: "weather.query", confidence: 0.82, toolInput: { location: extractWeatherLocation(raw) }, reason: "weather keywords", ...modelMeta("Orchestrator", "heuristic", "Local Heuristic") }; if (/哪个城市.*天气|想查.*天气|城市.*天气/u.test(prevAssistant) && /^[\u4e00-\u9fa5A-Za-z .·-]{2,16}(吧|呗|呀|呢)?$/u.test(clean)) return { intent: "weather.query", confidence: 0.86, toolInput: { location: cleanWeatherLocation(raw) }, reason: "weather location follow-up", ...modelMeta("Orchestrator", "heuristic", "Local Heuristic") }; if (/(新闻|大新闻|热点|今日热点|最新|最近|搜索|搜一下|查一下|查查|上网查|联网查|资料|官网|价格|榜单|实时|现在发生)/u.test(raw) && !/(导航|记账|提醒|闹钟|打开|保存|设置家|回家)/u.test(raw)) return { intent: /新闻|热点/u.test(raw) ? "news.query" : "web.search", confidence: 0.82, toolInput: { query: cleanSearchQuery(raw) }, reason: "web search keywords", ...modelMeta("Orchestrator", "heuristic", "Local Heuristic") }; if (/(导航|路线|怎么走|去|到|回家|去学校|去公司|去宿舍)/u.test(raw)) return { intent: "navigation.start", confidence: 0.72, toolInput: {}, reason: "navigation-like", ...modelMeta("Orchestrator", "heuristic", "Local Heuristic") }; if (/(改成|换成|少步行|避开高速|换高德|换百度)/u.test(raw)) return { intent: "navigation.modify", confidence: 0.76, toolInput: {}, reason: "navigation modify-like", ...modelMeta("Orchestrator", "heuristic", "Local Heuristic") }; if (/(家就是|把家设为|学校就是|公司就是|宿舍就是|默认用|默认坐|默认公交|默认地铁)/u.test(raw)) return { intent: "navigation.preference.set", confidence: 0.9, toolInput: {}, reason: "navigation preference", ...modelMeta("Orchestrator", "heuristic", "Local Heuristic") }; if (/(闹钟|叫我|提醒我|提醒一下|叫醒|起床)/u.test(raw)) return { intent: "alarm.set", confidence: 0.9, toolInput: {}, reason: "alarm", ...modelMeta("Orchestrator", "heuristic", "Local Heuristic") }; if (/^(打开|启动|帮我打开)\s*[\u4e00-\u9fa5A-Za-z0-9]+$/u.test(raw)) return { intent: "app.open", confidence: 0.9, toolInput: {}, reason: "open app", ...modelMeta("Orchestrator", "heuristic", "Local Heuristic") }; if (/(花了|消费|支出|收入|收到|工资|报销|午饭|晚饭|早餐|奶茶|打车).*(\d+(?:\.\d+)?)/u.test(raw)) return { intent: "ledger.create", confidence: 0.78, toolInput: {}, reason: "ledger", ...modelMeta("Orchestrator", "heuristic", "Local Heuristic") }; return { intent: "chat", confidence: 0.55, toolInput: {}, reason: "default", ...modelMeta("Orchestrator", "heuristic", "Local Heuristic") }; }
...
async function weather(locationOrText, memory = {}) { const location = cleanWeatherLocation(extractWeatherLocation(locationOrText)) || cleanWeatherLocation(memory.currentCity || memory.currentLocation); if (!location) return baseResponse("你想查哪个城市的天气？比如可以说：重庆今天会下雨吗。", "weather_need_location", modelMeta("Open-Meteo", "weather-forecast", "Open-Meteo")); try { const place = await geocode(location); if (!place) return baseResponse(`我没有找到“${location}”的天气位置。可以换成更明确的城市名，比如“重庆市天气”。`, "weather_tool", modelMeta("Open-Meteo", "geocoding+forecast", "Open-Meteo")); const f = await fetchJson(`https://api.open-meteo.com/v1/forecast?latitude=${place.latitude}&longitude=${place.longitude}&current=temperature_2m,apparent_temperature,precipitation,weather_code,wind_speed_10m&daily=temperature_2m_max,temperature_2m_min,precipitation_probability_max&forecast_days=3&timezone=auto`); const c = f.current || {}; const d = f.daily || {}; const rain = d.precipitation_probability_max?.[0]; return baseResponse(`${place.name}${place.admin1 ? `（${place.admin1}）` : ""}当前约 ${round(c.temperature_2m)}℃，体感 ${round(c.apparent_temperature)}℃，${weatherText(c.weather_code)}，风速约 ${round(c.wind_speed_10m)} km/h。今天气温约 ${round(d.temperature_2m_min?.[0])}–${round(d.temperature_2m_max?.[0])}℃，最高降水概率约 ${rain ?? "未知"}%。${Number(rain) >= 50 || Number(c.precipitation) > 0 ? "建议带伞。" : "降雨风险不算高。"}`, memory.currentCity ? "weather_tool_memory" : "weather_tool", modelMeta("Open-Meteo", "forecast-v1", "Open-Meteo")); } catch { return baseResponse("天气接口暂时请求失败。你可以稍后再试，或换一个更明确的城市名。", "weather_error", modelMeta("Open-Meteo", "forecast-v1", "Open-Meteo")); } }
...
async function searchWeb(env, rawQuery, modelPreference = "auto") {
  ...
  try {
    const res = await fetch("https://api.tavily.com/search", { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${env.TAVILY_API_KEY}` }, body: JSON.stringify({ query, search_depth: "advanced", topic: "general", max_results: 6, include_answer: true, include_raw_content: false, include_images: false }) });
...
async function chatWithProviderPool(env, body, text, memory = {}, modelPreference = "auto") { const messages = Array.isArray(body?.messages) ? body.messages.slice(-12) : []; const conversation = messages.map((m) => `${m?.role === "assistant" ? "助手" : "用户"}：${String(m?.content || "").slice(0, 1200)}`).join("\n"); const prompt = ["你是手机端 AI 助手的普通聊天层。", "只回答普通聊天、学习解释、概念说明和非本地执行类问题。", "可以参考本地记忆理解上下文，但不要无故复述隐私记忆。", "不要生成 command/mobileCommand，不要声称已经执行手机动作。", "回答用中文，简洁自然；必要时分点，但不要太啰嗦。", "本地记忆：", JSON.stringify(memory || {}), "对话历史：", conversation || `用户：${text}`].join("\n"); const result = await callTextProviderPool(env, prompt, { task: "chat", maxTokens: 1200, temperature: 0.35, modelPreference }); if (result?.text) return baseResponse(result.text.trim(), result.source, result.meta); return baseResponse("云端 AI 暂时不可用：Gemini、NVIDIA 和 Workers AI 都没有成功返回。", "provider_pool_failed", modelMeta("Provider Pool", "all_failed", "多云端 AI 均不可用")); }
async function callTextProviderPool(env, prompt, options = {}) { const errors = []; const preferred = normalizeModelPreference(options.modelPreference); for (const provider of orderProviders(providerPool(env), preferred).filter((item) => item.tasks.includes(options.task || "chat") || item.tasks.includes("chat"))) { try { let text = ""; if (provider.kind === "nvidia") text = await callNvidiaChat(env, provider.model, [{ role: "system", content: "你是中文手机 AI 助手。回答要清楚、简洁、可靠。" }, { role: "user", content: prompt }], options.maxTokens || 1000, options.temperature ?? 0.35); else if (provider.kind === "gemini") text = await callGeminiText(env, provider.model, prompt, options.maxTokens || 1000, options.temperature ?? 0.35, false); else if (provider.kind === "workers_ai") text = await callWorkersAIText(env, prompt, options.maxTokens || 800); if (String(text || "").trim()) return { text: String(text).trim(), source: provider.source, meta: modelMeta(provider.provider, provider.model, provider.label) }; } catch (error) { errors.push(`${provider.label}: ${String(error?.message || error).slice(0, 80)}`); } } return { text: "", errors }; }
function providerPool(env) { const pool = []; if (env.NVIDIA_API_KEY) pool.push({ id: "kimi", kind: "nvidia", provider: "NVIDIA NIM", model: nvidiaChatModel(env), label: nvidiaModelLabel(nvidiaChatModel(env)), source: "nvidia_chat", tasks: ["chat", "search_summary"] }); if (env.GEMINI_API_KEY) pool.push({ id: "gemini", kind: "gemini", provider: "Gemini", model: geminiModel(env), label: geminiModelLabel(geminiModel(env)), source: "gemini_chat", tasks: ["chat", "search_summary"] }); if (env.AI) pool.push({ id: "workers", kind: "workers_ai", provider: "Cloudflare Workers AI", model: String(env.AI_MODEL || "@cf/meta/llama-3.1-8b-instruct"), label: "Workers AI Llama 3.1 8B", source: "workers_ai_text_fallback", tasks: ["chat", "search_summary"] }); return pool; }
function selectableModels(env) { return [{ id: "auto", label: "自动", description: "按可用额度自动切换" }, ...providerPool(env).map((p) => ({ id: p.id, label: p.label, provider: p.provider, model: p.model }))]; }
function normalizeModelPreference(value) { const v = String(value || "auto").toLowerCase().trim(); if (["auto", "gemini", "kimi", "nvidia", "workers", "workers_ai"].includes(v)) return v === "nvidia" ? "kimi" : v === "workers_ai" ? "workers" : v; return "auto"; }
function orderProviders(pool, preferred = "auto") { if (!preferred || preferred === "auto") return pool; const p = normalizeModelPreference(preferred); return [...pool].sort((a, b) => (a.id === p ? -1 : b.id === p ? 1 : 0)); }
async function callNvidiaChat(env, model, messages, maxTokens = 1000, temperature = 0.35) { const endpoint = `${nvidiaBaseUrl(env).replace(/\/+$/g, "")}/chat/completions`; const res = await fetch(endpoint, { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${env.NVIDIA_API_KEY}` }, body: JSON.stringify({ model, messages, temperature, max_tokens: maxTokens }) }); const data = await res.json().catch(() => null); if (!res.ok) throw new Error(data?.error?.message || data?.message || `NVIDIA HTTP ${res.status}`); return data?.choices?.[0]?.message?.content || data?.choices?.[0]?.text || ""; }
async function callGeminiText(env, model, prompt, maxTokens = 1000, temperature = 0.35, jsonMode = false) { const endpoint = `${env.GEMINI_API_BASE || "https://generativelanguage.googleapis.com/v1beta/models"}/${model}:generateContent?key=${encodeURIComponent(env.GEMINI_API_KEY)}`; const generationConfig = { temperature, maxOutputTokens: maxTokens }; if (jsonMode) generationConfig.responseMimeType = "application/json"; const res = await fetch(endpoint, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: prompt }] }], generationConfig }) }); const data = await res.json().catch(() => null); if (!res.ok) throw new Error(data?.error?.message || `Gemini HTTP ${res.status}`); return data?.candidates?.[0]?.content?.parts?.map((p) => p.text || "").join("\n").trim() || ""; }
async function callWorkersAIText(env, prompt, maxTokens = 800) { if (!env.AI) throw new Error("Workers AI binding is not available"); const result = await env.AI.run(String(env.AI_MODEL || "@cf/meta/llama-3.1-8b-instruct"), { messages: [{ role: "system", content: "你是中文手机 AI 助手。回答简洁可靠。" }, { role: "user", content: prompt }], max_tokens: maxTokens }); return String(result?.response || result?.text || "").trim(); }
function formatSourceLinks(sources) { const lines = sources.filter((s) => s.url).slice(0, 4).map((s) => `- ${s.title || "来源"}：${s.url}`); return lines.length ? `参考网址：\n${lines.join("\n")}` : ""; }
function ensureSourceLinks(reply, sources) { const text = String(reply || "").trim(); const links = formatSourceLinks(sources); if (!links) return text; if (/参考网址|来源网址|参考链接|来源链接/u.test(text) && sources.some((s) => s.url && text.includes(s.url))) return text; return `${text}\n\n${links}`; }
...
function trim(text, max = 120) { const value = String(text || "").replace(/\s+/g, " ").trim(); return value.length > max ? `${value.slice(0, max)}…` : value; }
