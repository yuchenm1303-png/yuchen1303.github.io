...
class DeviceControlActionVerifierTest {
    ...
    fun extractsExecutorConfirmedTargetPercent() {
        assertEquals(55f, targetPercentFromExecutionMessage("已把屏幕亮度从约 40% 调到约 55%。"))
        assertEquals(72f, targetPercentFromExecutionMessage("已把媒体音量从约 57% 调到约 72%。"))
        assertEquals(37.5f, targetPercentFromExecutionMessage("已调到约 37.5%。"))
    }
