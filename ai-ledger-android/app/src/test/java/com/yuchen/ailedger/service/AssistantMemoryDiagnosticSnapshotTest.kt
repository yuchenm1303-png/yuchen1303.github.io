...
class AssistantMemoryDiagnosticSnapshotTest {
    ...
    @Test
    fun responseSnapshotDeepCopiesNestedMemoryMetadata() {
        val trace = JSONObject().put("prompt", "测试问题")
        val mutation = JSONObject().put("status", "applied")
        val source = JSONObject()
            .put("memoryStatus", "used")
            .put("memoryTrace", trace)
            .put("memoryMutation", mutation)
            .put("sources", JSONArray().put("unrelated"))

        val snapshot = compactMemoryDiagnosticResponse(source)
        trace.put("prompt", "已被网络线程修改")
        mutation.put("status", "changed")

        assertEquals("used", snapshot.optString("memoryStatus"))
        assertNotSame(trace, snapshot.optJSONObject("memoryTrace"))
        assertEquals("测试问题", snapshot.optJSONObject("memoryTrace")?.optString("prompt"))
        assertEquals("applied", snapshot.optJSONObject("memoryMutation")?.optString("status"))
