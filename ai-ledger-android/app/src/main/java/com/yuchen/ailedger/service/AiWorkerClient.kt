...
class AiWorkerClient(private val config: AiWorkerConfig = AiWorkerConfig()) {
    ...
    private fun buildPayload(messages: List<ChatMessage>, route: ModelRoute, onlineEnabled: Boolean): JSONObject {
        ...
        return JSONObject().apply {
            ...
            put("commandProtocol", JSONObject().apply { put("enabled", true); put("version", 4); put("client", "android-compose"); put("agentModeEnabled", agentModeEnabled); put("agentStartRequested", shouldStartAgent); put("returnNaturalReply", true); put("requireConfirmationForActions", true); put("supportedAgentActions", JSONArray(listOf("observe_screen", "run_agent_task"))); put("supportedMobileActions", JSONArray(listOf("set_alarm", "navigate"))); put("supportedPreferenceUpdates", JSONArray(listOf("navigation_address"))); put("navigationAddressSlots", JSONArray(listOf("home", "school", "company", "dorm"))); put("fallbackTransport", "embedded_marker"); put("embeddedMarker", "[[AI_LEDGER_COMMAND:{...}]]") })
