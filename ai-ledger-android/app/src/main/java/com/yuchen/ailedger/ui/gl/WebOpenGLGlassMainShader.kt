...
internal object WebOpenGLGlassMainShader {
    const val BODY_PREFIX = """
        void main(){
            ...
            if(press>0.0){
                ...
                pressWide=press*pow(
                    sat(1.0-length((p*rectInv-pressCenter)*vec2(pressAspect,1.0))*0.58),
                    1.25
                );
            ...
            vec2 centerFlow=centerTransport(
                p,center,invSafeCenter,minSize,transportParams
            );
            ...
            if(depth<width){
                ...
                vec4 shoulderData=evaluateShoulderSource(
                    p,normal,center,safeHalfSize,sdfCore,invSafeCenter,r,depth,
                    shoulderGeometry,transportParams.y,sourceDepth,sourceNormal
                );
                ...
                bodyOpticalCoord=evaluateBodyOpticalCoordAt(
                    sourcePoint,sourceDepth,sourceNormal,z,rectInv,center,
                    invSafeCenter,minSize,lensParams,sourceWeight,
                    transportParams,pressOptics
                );
                shoulderActive=true;
            }
    ...
    const val BODY_SUFFIX = """
            ...
            if(dispersionStrength>0.001&&dispersionDistance>0.001){
                ...
                float dispersionMask=pow(edgeEnvelope,resolvedDispersion.w)
                ...
                if(dispersionMask>0.001){
                    ...
                    vec3 redSample=blurPyramidBackdropAt(
                        globalUvAt(bodyOpticalCoord+splitPx,rootInv,uvTexel),
                        bodyBlurAmount
                    );
                    vec3 blueSample=blurPyramidBackdropAt(
                        globalUvAt(bodyOpticalCoord-splitPx,rootInv,uvTexel),
                        bodyBlurAmount
                    );
            ...
            if(shoulderActive){
                ...
                float fillSheen=sat(
                    0.072*strength*fill*(0.40+0.60*lightFacing)
                );
