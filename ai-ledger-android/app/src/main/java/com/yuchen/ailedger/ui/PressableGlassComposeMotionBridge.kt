 ...
 * 普通玻璃真实光动效桥接。
 *
 * 这里不再额外绘制任何光圈、边框、扫光或独立形变，避免外层坐标和玻璃本体不同步。
 * 光效、按压形变、释放回弹全部交回 Glass.kt 原生 PressableGlass 链处理，使用同一组
...
fun PressableGlass(
    ...
    OrdinaryGlassNativeTempo(motion) {
        PressableGlass(
            quality = quality,
            glassIntensity = glassIntensity,
...
@Composable
private fun OrdinaryGlassNativeTempo(
    motion: ComposeGlassMotionStyle,
    content: @Composable () -> Unit
) {
    val durationScale = ordinaryDurationScaleForSpeed(motion.speed)
    val scale = remember(durationScale) {
        object : MotionDurationScale {
            override val scaleFactor: Float = durationScale
        }
    }
    CompositionLocalProvider(LocalMotionDurationScale provides scale) {
        content()
    }
}
...
private fun ordinaryDurationScaleForSpeed(speed: Float): Float {
    val cleanSpeed = speed.coerceIn(0.35f, 2.5f)
    return (1f / cleanSpeed).coerceIn(0.40f, 2.85f)
}
