...
object AgentDeviceContextProvider {
    ...
    fun build(
        ...
        val compactVisualHotPath = shouldUseCompactVisualContext(contract, targetResolution)

        val candidateStartedAt = SystemClock.elapsedRealtime()
        val candidateApps = if (compactVisualHotPath) {
            targetResolution.selectedApp?.let(::listOf).orEmpty()
        } else {
            installedAppIndex.findCandidateApps(goal, limit = MAX_CANDIDATE_APPS)
        }
        val candidateResolveMs = elapsedSince(candidateStartedAt)

        val hashStartedAt = SystemClock.elapsedRealtime()
        val inventoryHash = buildInventoryHash(allLaunchableApps)
        val inventoryApps = if (compactVisualHotPath) {
            targetResolution.selectedApp?.let(::listOf).orEmpty()
        } else {
            mergePriorityApps(candidateApps, allLaunchableApps, MAX_LAUNCHABLE_APPS)
        }
        ...
        val operationMemory = if (compactVisualHotPath) {
            JSONObject().apply {
                put("schema", "agent_operation_memory_reference_v1")
                put("omittedInVisualHotPath", true)
            }
        } else {
            AgentOperationMemory.build(
                goal = goal,
        ...
        val shellStatus = if (compactVisualHotPath) null else runCatching { shellBridge.probe() }.getOrNull()
        val shellProbeMs = elapsedSince(shellStartedAt)
        val clientPerformance = JSONObject().apply {
            put("schema", "android_agent_client_performance_v2")
            put("compactVisualHotPath", compactVisualHotPath)
        ...
        val json = JSONObject().apply {
            put("schema", "android_device_context_v7_lean_visual")
            put("inventory", JSONObject().apply {
                put("schema", "android_launchable_app_inventory_v3")
                put("inventoryHash", inventoryHash)
                put("installedAppCount", allLaunchableApps.size)
                put("uploadedAppCount", inventoryApps.size)
                put("truncated", compactVisualHotPath || allLaunchableApps.size > inventoryApps.size)
                put("referenceOnly", compactVisualHotPath)
            ...
            put("installedAppCount", allLaunchableApps.size)
            put("uploadedAppCount", inventoryApps.size)
            put("installedAppsTruncated", compactVisualHotPath || allLaunchableApps.size > inventoryApps.size)
            put("appInventoryHash", inventoryHash)
            put("visualHotPath", compactVisualHotPath)
            contract?.let {
            ...
            put("localUserMemory", if (compactVisualHotPath) {
                JSONObject().apply {
                    put("schema", "assistant_local_memory_reference_v1")
                    put("hasNavigationMemory", localMemory.hasNavigationMemory)
                }
            } else {
                localMemory.toJson()
            })
        ...
        val summary = buildString {
            ...
            if (compactVisualHotPath) append("，视觉热路径已压缩")
    ...
    private fun shouldUseCompactVisualContext(
        contract: AgentTaskExecutionContract?,
        targetResolution: TargetAppResolution,
    ): Boolean {
        if (contract == null || targetResolution.status != "resolved") return false
        return contract.phase == AGENT_TASK_PHASE_VISUAL_NAVIGATION ||
            contract.phase == AGENT_TASK_PHASE_VERIFY_RESULT ||
            contract.phase == AGENT_TASK_PHASE_VERIFY_TARGET_APP
    }
