...
internal class VisualAgentHudHost(
    ...
    private fun createOverlay(): Boolean {
        ...
        val view = WebView(service).apply {
            ...
            setOnGenericMotionListener { _, _ -> false }
        ...
        val params = WindowManager.LayoutParams(
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                // Use the supported inset contract instead of FLAG_LAYOUT_NO_LIMITS. This keeps one
                // physical full-display WebView while avoiding OEM-specific oversized input frames.
                setFitInsetsTypes(0)
                setFitInsetsIgnoringVisibility(true)
            }
            applyPassthroughWindowContract(this)
        }
    ...
    private fun applyPassthroughWindowContract(params: WindowManager.LayoutParams) {
        params.flags = params.flags or REQUIRED_PASSTHROUGH_FLAGS
        params.softInputMode = WindowManager.LayoutParams.SOFT_INPUT_ADJUST_NOTHING
    }

    private fun reinforcePassthroughWindowContract() {
        val view = webView ?: return
        val params = layoutParams ?: return
        applyPassthroughWindowContract(params)
