...
object DeviceControlCapabilityRegistry {
    val capabilities: List<DeviceControlCapability> = listOf(
        DeviceControlCapability(
            ...
            description = "读取电量、内存、存储、网络、媒体音量、自动旋转和可启动应用数量，不操作其他应用。",
        ...
        DeviceControlCapability(
            ...
            status = "implemented",
            riskLevel = DeviceControlRiskLevel.Low,
            description = "通过 Android Settings Intent 直达 Wi‑Fi、蓝牙、声音、显示、勿扰、电池、存储、无障碍、开发者选项等页面。",
        ...
        DeviceControlCapability(
            ...
            description = "用户授予修改系统设置后，可直接写入屏幕亮度并读回验证。",
        ...
        DeviceControlCapability(
            ...
            status = "implemented_requires_write_settings",
            riskLevel = DeviceControlRiskLevel.Medium,
            description = "用户授予修改系统设置后，可直接写入 SCREEN_OFF_TIMEOUT 并读回验证。",
            examples = listOf("自动锁屏改成 1 分钟"),
        ),
        DeviceControlCapability(
            id = "system.auto_rotate",
            title = "设置自动旋转",
            status = "implemented_requires_write_settings",
            riskLevel = DeviceControlRiskLevel.Medium,
            description = "用户授予修改系统设置后，可直接开启或关闭自动旋转并读回验证。",
            examples = listOf("打开自动旋转", "关闭屏幕旋转"),
        ),
        DeviceControlCapability(
            id = "system.media_volume",
            title = "设置媒体音量",
            status = "implemented",
            riskLevel = DeviceControlRiskLevel.Low,
            description = "通过 AudioManager 调节媒体音量，并用当前音量读回验证。",
            examples = listOf("把媒体音量调到 30%", "音量调高一点"),
        ),
        DeviceControlCapability(
            id = "shell.probe",
        ...
        DeviceControlCapability(
            id = "network.wifi_toggle",
            title = "开启/关闭 Wi‑Fi",
            status = "implemented_requires_shizuku_with_settings_fallback",
            riskLevel = DeviceControlRiskLevel.Medium,
            description = "Shizuku/ADB 增强身份可执行 svc wifi enable/disable；无增强权限时打开 Wi‑Fi 设置页兜底。",
            examples = listOf("打开 Wi-Fi", "关闭 Wi-Fi"),
        ),
        DeviceControlCapability(
            id = "network.bluetooth_toggle",
            title = "开启/关闭蓝牙",
            status = "implemented_requires_shizuku_with_settings_fallback",
            riskLevel = DeviceControlRiskLevel.Medium,
            description = "Shizuku/ADB 增强身份可执行蓝牙开关命令；无增强权限时打开蓝牙设置页兜底。",
            examples = listOf("打开蓝牙", "关闭蓝牙"),
        ),
        DeviceControlCapability(
            id = "network.mobile_data_toggle",
            title = "开启/关闭移动数据",
            status = "implemented_requires_shizuku_with_settings_fallback",
            riskLevel = DeviceControlRiskLevel.Medium,
            description = "Shizuku/ADB 增强身份可执行 svc data enable/disable；无增强权限时打开流量设置页兜底。",
            examples = listOf("打开移动数据", "关闭流量"),
        ),
        DeviceControlCapability(
            id = "system.dark_mode",
            title = "设置深色模式",
            status = "implemented_requires_shizuku_with_settings_fallback",
            riskLevel = DeviceControlRiskLevel.Medium,
            description = "Shizuku/ADB 增强身份可执行 cmd uimode night；无增强权限时打开显示设置页兜底。",
            examples = listOf("打开深色模式", "关闭深色模式", "深色模式跟随系统"),
        ),
        DeviceControlCapability(
            id = "app.force_stop",
            title = "强停应用",
