...
internal class VisualAgentHudOverlayView(context: Context) : View(context) {
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        shaderSize = 0L
        if (toX == 0f && toY == 0f) {
            fromX = w * .52f; fromY = h * .46f; toX = fromX; toY = fromY
        }
    ...
    private fun syncAnimation(now: Long) {
        ...
        if (visibleNow && !animating) {
            ...
            removeCallbacks(frame)
            post(frame)
        } else if (!visibleNow) stopAnimation()
    }

    private val frame = object : Runnable {
        override fun run() { if (animating) invalidate() }
    }
    ...
    private fun phase(now: Long): VisualHudPhase {
        ...
        if (target?.actionType in setOf("tap_xy", "tap_node") && last == progress.currentAction) {
            return if (now - progress.updatedAt < MOVE_MS) VisualHudPhase.Moving else VisualHudPhase.Clicking
        }
    ...
    private fun drawEdge(canvas: Canvas, now: Long, alpha: Float) {
        ...
        edgeLayers(canvas, cast, 18, .8f * breath * alpha, 2.35f)
        edgeLayers(canvas, halo, 12, .58f * breath * alpha, 1.35f)
    ...
    private fun edgeLayers(canvas: Canvas, depth: Float, layers: Int, opacity: Float, power: Float) {
        ...
        repeat(layers) { i ->
            ...
            edgePaint.alpha = (opacity * (1f - t).coerceAtLeast(0f).pow(power) * 255f).toInt().coerceIn(0, 255)
    ...
    private fun edgeShader(): SweepGradient {
        ...
        if (edgeShader == null || key != shaderSize) {
            ...
            edgeShader = SweepGradient(
                width / 2f, height / 2f,
                intArrayOf(
                    Color.rgb(255, 57, 174), Color.rgb(255, 116, 64), Color.rgb(255, 229, 70),
                    Color.rgb(91, 239, 138), Color.rgb(49, 224, 229), Color.rgb(85, 132, 255),
                    Color.rgb(166, 82, 255), Color.rgb(255, 57, 174),
                ),
    ...
    private fun drawStatus(canvas: Canvas, phase: VisualHudPhase, alpha: Float) {
        ...
        fill.shader = LinearGradient(rect.left, rect.top, rect.right, rect.bottom,
            intArrayOf(Color.argb((226 * alpha).toInt(), 7, 18, 32), Color.argb((214 * alpha).toInt(), 19, 31, 50)),
            null, Shader.TileMode.CLAMP)
        canvas.drawRoundRect(rect, h / 2f, h / 2f, fill)
        fill.shader = null
        stroke.strokeWidth = dp(1f); stroke.color = Color.argb((76 * alpha).toInt(), 144, 229, 255)
    ...
    private fun drawStages(canvas: Canvas, phase: VisualHudPhase, alpha: Float) {
        ...
        val w = min(width * .82f, dp(360f)); val h = dp(38f)
        ...
        canvas.drawRoundRect(rect, h / 2f, h / 2f, fill)
        stroke.strokeWidth = dp(1f); stroke.color = Color.argb((62 * alpha).toInt(), 119, 209, 239)
        canvas.drawRoundRect(rect, h / 2f, h / 2f, stroke)
        text.textSize = sp(10.5f)
        val itemW = rect.width() / labels.size
        labels.forEachIndexed { i, label ->
            val cx = rect.left + itemW * (i + .5f)
            val active = i == phase.index; val done = i < phase.index
            val c = when { active -> Color.argb((248 * alpha).toInt(), 238, 252, 255)
                done -> Color.argb((232 * alpha).toInt(), 91, 242, 186)
                else -> Color.argb((145 * alpha).toInt(), 155, 187, 202) }
            fill.color = c; canvas.drawCircle(cx - dp(15f), rect.centerY(), if (active) dp(3.2f) else dp(2.4f), fill)
            text.color = c
            canvas.drawText(label, cx + dp(3f) - text.measureText(label) / 2f, baseline(rect.centerY(), text), text)
        }
    }

    private fun drawCard(canvas: Canvas, x: Float, y: Float, phase: VisualHudPhase, alpha: Float) {
        val w = min(width * .72f, dp(320f)); val h = dp(132f); val gap = dp(18f)
        ...
        fill.shader = LinearGradient(r.left, r.top, r.right, r.bottom,
            intArrayOf(Color.argb((235 * alpha).toInt(), 5, 18, 31), Color.argb((225 * alpha).toInt(), 8, 30, 43)),
            null, Shader.TileMode.CLAMP)
        canvas.drawRoundRect(r, dp(18f), dp(18f), fill); fill.shader = null
        stroke.strokeWidth = dp(1f); stroke.color = Color.argb((82 * alpha).toInt(), 102, 220, 255)
        canvas.drawRoundRect(r, dp(18f), dp(18f), stroke)
        val pad = dp(14f)
        bold.textSize = sp(11.8f); bold.color = Color.argb((250 * alpha).toInt(), 239, 250, 255)
        canvas.drawText(headline(phase), r.left + pad, r.top + dp(25f), bold)
        text.textSize = sp(10.2f); text.color = Color.argb((205 * alpha).toInt(), 182, 221, 237)
        canvas.drawText(progress.currentAction.ifBlank { "正在执行视觉任务" }.oneLine().cut(44), r.left + pad, r.top + dp(49f), text)
        text.color = Color.argb((232 * alpha).toInt(), 221, 240, 248)
        val targetLabel = target?.targetText?.takeIf(String::isNotBlank)?.let { "目标：${it.oneLine().cut(34)}" } ?: "目标：当前页面"
        canvas.drawText(targetLabel, r.left + pad, r.top + dp(72f), text)
        text.color = Color.argb((180 * alpha).toInt(), 160, 202, 220)
        val detail = when { target?.detail?.isNotBlank() == true -> target!!.detail
            progress.lastResult.isNotBlank() -> progress.lastResult
            else -> "正在根据页面证据选择下一步操作" }.oneLine().cut(54)
        canvas.drawText(detail, r.left + pad, r.top + dp(96f), text)
        text.color = accent(phase, (220 * alpha).toInt())
        canvas.drawText(progress.lastResult.takeIf(String::isNotBlank)?.oneLine()?.cut(48) ?: "等待动作结果",
            r.left + pad, r.top + dp(119f), text)
    }

    private fun drawCursor(canvas: Canvas, x: Float, y: Float, now: Long, phase: VisualHudPhase, alpha: Float) {
        val s = max(width / DESIGN_W, .64f)
        val left = x - 10f * s - .5f * s
        val top = y - 10.5f * s * .95f - .5f * s
        val path = cursorPath(s)
        path.transform(Matrix().apply {
            postRotate(-2.5f, 10f * s, 10.5f * s * .95f); postTranslate(left, top)
        })
        stroke.strokeWidth = max(1f, 7f * s); stroke.color = Color.argb((48 * alpha).toInt(), 87, 225, 255)
        canvas.drawPath(path, stroke)
        stroke.strokeWidth = max(1f, 3.2f * s); stroke.color = Color.argb((44 * alpha).toInt(), 255, 88, 201)
        canvas.drawPath(path, stroke)
        val save = canvas.save(); canvas.clipPath(path)
        fill.color = Color.argb((238 * alpha).toInt(), 8, 17, 33)
        canvas.drawRect(left - 8f, top - 8f, left + 80f * s, top + 80f * s, fill)
        radial(canvas, left, top, s, 33.7f, 16f, 23f, Color.rgb(63, 226, 255), .92f * alpha)
        radial(canvas, left, top, s, 29f, 28f, 18f, Color.WHITE, .86f * alpha)
        radial(canvas, left, top, s, 30f, 50f, 18f, Color.rgb(255, 65, 190), .72f * alpha)
        radial(canvas, left, top, s, 28.2f, 28.1f, 11.8f, Color.WHITE, .09f * alpha)
        canvas.restoreToCount(save)
        stroke.strokeWidth = max(1f, 1.02f * s); stroke.color = Color.argb((242 * alpha).toInt(), 213, 252, 255)
        canvas.drawPath(path, stroke)
        stroke.strokeWidth = max(.8f, .42f * s); stroke.color = Color.argb((220 * alpha).toInt(), 255, 255, 255)
        canvas.drawPath(path, stroke)
        val residual = now - resultPulseAt
        if (phase == VisualHudPhase.Clicking || residual in 0L until 620L) {
            val elapsed = if (phase == VisualHudPhase.Clicking) (now - progress.updatedAt - MOVE_MS).coerceAtLeast(0L) else residual
            val t = (elapsed % 620L) / 620f
            stroke.strokeWidth = dp(1.5f); stroke.color = Color.argb(((1f - t) * 175f * alpha).toInt(), 164, 242, 255)
    ...
    private fun cursorPath(s: Float) = Path().apply {
        moveTo(7.8f*s, 7.8f*s*.95f)
        cubicTo(8.91f*s,5.71f*s*.95f,13.83f*s,7.17f*s*.95f,20.4f*s,10.4f*s*.95f)
        cubicTo(26.97f*s,13.63f*s*.95f,49.02f*s,25.34f*s*.95f,51.1f*s,29.1f*s*.95f)
        cubicTo(53.18f*s,32.86f*s*.95f,37.42f*s,31.38f*s*.95f,34.1f*s,35.2f*s*.95f)
        cubicTo(30.78f*s,39.02f*s*.95f,30.93f*s,52.78f*s*.95f,29.2f*s,54.3f*s*.95f)
        cubicTo(27.47f*s,55.82f*s*.95f,25.14f*s,49.77f*s*.95f,22.7f*s,45.2f*s*.95f)
        cubicTo(20.26f*s,40.63f*s*.95f,15.36f*s,29.87f*s*.95f,13.1f*s,24.2f*s*.95f)
        cubicTo(10.84f*s,18.53f*s*.95f,6.69f*s,9.89f*s*.95f,7.8f*s,7.8f*s*.95f); close()
    }

    private fun radial(canvas: Canvas, left: Float, top: Float, s: Float, cx: Float, cy: Float, radius: Float, color: Int, opacity: Float) {
        fill.shader = RadialGradient(left + cx*s, top + cy*s*.95f, max(1f, radius*s),
            Color.argb((opacity.coerceIn(0f,1f)*255).toInt(), Color.red(color), Color.green(color), Color.blue(color)),
            Color.TRANSPARENT, Shader.TileMode.CLAMP)
        canvas.drawCircle(left + cx*s, top + cy*s*.95f, radius*s, fill); fill.shader = null
    }

    private fun headline(p: VisualHudPhase) = when (p) {
        VisualHudPhase.Observing -> "正在观察当前页面"; VisualHudPhase.Analyzing -> "正在分析页面证据"
        VisualHudPhase.Moving -> "正在移动到目标位置"; VisualHudPhase.Clicking -> "正在执行点击操作"
        VisualHudPhase.Verifying -> "正在验证执行结果"; VisualHudPhase.Paused -> "等待用户继续"
        VisualHudPhase.Completed -> "本次动作已结束"
    }

    private fun accent(p: VisualHudPhase, a: Int) = when (p) {
        VisualHudPhase.Paused -> Color.argb(a,255,213,116)
        VisualHudPhase.Completed -> Color.argb(a,111,244,186)
        VisualHudPhase.Clicking -> Color.argb(a,255,225,248)
        else -> Color.argb(a,103,232,255)
    }

    private fun baseline(cy: Float, p: Paint) = cy - (p.ascent() + p.descent()) / 2f
    private fun lerp(a: Float, b: Float, t: Float) = a + (b - a) * t
    private fun dp(v: Float) = v * density
    private fun sp(v: Float) = v * scaledDensity
    private fun String.oneLine() = trim().replace(Regex("\\s+"), " ")
    private fun String.cut(n: Int) = if (length <= n) this else take((n - 1).coerceAtLeast(1)).trimEnd() + "…"

    companion object { private const val DESIGN_W = 1600f; private const val MOVE_MS = 90L }
}
