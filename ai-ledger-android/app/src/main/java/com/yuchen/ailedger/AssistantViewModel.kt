...
private const val ASSISTANT_IMAGE_MIN_JPEG_QUALITY = 60
private const val ASSISTANT_IMAGE_QUALITY_STEP = 8
private const val ASSISTANT_IMAGE_MAX_BYTES = 4 * 1024 * 1024
private const val ASSISTANT_IMAGE_TOTAL_MAX_BYTES = 10 * 1024 * 1024
...
class AssistantViewModel(
    ...
    fun sendUserCommand(text: String) {
        ...
        val attachments = uiState.composerAttachments.mapNotNull {
            it.takeIf { item -> item.isReady }?.toChatAttachment()
        }
        if (uiState.isSending || hasPreparingAttachment) return
        if (cleanText.isBlank() && attachments.isEmpty()) return

        val totalAttachmentBytes = attachments.sumOf { it.sizeBytes?.toLong() ?: 0L }
        if (totalAttachmentBytes > ASSISTANT_IMAGE_TOTAL_MAX_BYTES) {
            appendAssistantNotice("视觉附件总大小超过 10 MB，请移除部分图片后再发送。", source = "local")
            return
        }

        val baseText = cleanText.ifBlank { "请根据视觉附件进行分析。" }
        val userText = if (attachments.isNotEmpty()) {
            listOf(baseText, attachmentMessageFlag(attachments)).joinToString("\n\n")
        } else {
            baseText
        }
    ...
    fun requestAgentNextStep(goal: String) {
        ...
        uiState = uiState.copy(
            messages = uiState.messages + userMessage + pendingMessage,
            composerText = "",
            isSending = true
        )
    ...
    private suspend fun buildAgentTaskMessage(
        id: String,
        goal: String,
        ...
        if (executionMode != AgentExecutionMode.NormalChatDeviceTool && shouldOpenAccessibilityGuide(result)) {
    ...
    private fun buildAgentRunMessage(id: String, goal: String, result: AgentTaskRunResult): ChatMessage {
        ...
        val text = buildString {
            ...
            if (result.stoppedForConfirmation) {
    ...
    private fun buildMobileCommandPreviewMessage(id: String, command: MobileCommand): ChatMessage {
        return ChatMessage(
            id = id,
            text = commandReplyPrefix(command) +
                "\n\n动作：${command.title}\n详情：${command.summary}\n\n回复“确认”执行，或回复“取消”。",
            role = MessageRole.Assistant,
            source = "local_mobile",
            modelLabel = "待确认"
    ...
    private fun CloudAgentAction.isRunAgentTask(): Boolean {
        return capability == "run_agent_task" || capability == "agent_task"
    }

    private fun CloudAgentAction.isRunDeviceControl(): Boolean {
        return capability == "run_device_control" && deviceControlStep != null
    }

    private fun CloudAgentAction.isObserveScreen(): Boolean {
        return capability == "observe_screen"
    }

    private fun CloudAgentAction.resolvedGoal(fallback: String): String {
        return goal?.takeIf { it.isNotBlank() } ?: fallback
    }
    ...
    private fun applyCloudPreferenceUpdate(update: CloudPreferenceUpdate) {
        if (update.type == "navigation_address") updateNavigationAddress(update.slot, update.value)
    }

    fun retryMessage(messageId: String) {
        if (uiState.isSending) return
        val assistantIndex = uiState.messages.indexOfFirst {
            it.id == messageId && it.role == MessageRole.Assistant
        }
        if (assistantIndex <= 0) return
        val previousUser = uiState.messages.take(assistantIndex).lastOrNull {
            it.role == MessageRole.User && (it.text.isNotBlank() || it.hasImageAttachments)
        } ?: return
            ...
            try {
                val response = withContext(Dispatchers.IO) {
                    aiWorkerClient.streamChat(requestMessages, selectedModel, onlineEnabled) { delta ->
                        if (delta.isNotBlank()) {
                            synchronized(streamLock) { streamBuffer.append(delta) }
                        }
                    }
                }
                streamClosed = true
                ...
                if (activePendingMessageId == pendingMessage.id) {
                    ...
                    when {
                        cloudAgentAction?.isRunDeviceControl() == true -> {
                            replaceMessage(
                                pendingMessage.id,
                                buildDeviceControlMessage(pendingMessage.id, latestGoal, cloudAgentAction)
                            )
                        }
                        cloudAgentAction?.isRunAgentTask() == true -> {
                            replaceMessage(
                                pendingMessage.id,
                                buildAgentTaskMessage(
                                    pendingMessage.id,
                                    cloudAgentAction.resolvedGoal(latestGoal),
                                    AgentExecutionMode.ExplicitAgent
                                )
                            )
                        }
                        cloudAgentAction?.isObserveScreen() == true -> {
                            replaceMessage(pendingMessage.id, buildAgentObservationMessage(pendingMessage.id))
                        }
                        cloudCommand != null -> {
                            replaceMessage(
                                pendingMessage.id,
                                buildMobileCommandPreviewMessage(pendingMessage.id, cloudCommand)
                            )
                        }
                        else -> {
                            replaceMessage(
    ...
    fun toggleOnline() {
        ...
        appendAssistantNotice(
            if (enabled) "已开启联网开关。下一步会随请求传给 Worker。" else "已关闭联网开关。",
            source = "local"
        )
    }
    ...
    fun removeComposerAttachment(id: String) {
        if (!uiState.isSending) {
            uiState = uiState.copy(
                composerAttachments = uiState.composerAttachments.filterNot { it.id == id }
            )
        }
    }
        ...
        viewModelScope.launch {
            try {
                updateComposerAttachment(attachmentId) {
                    it.copy(progress = 0.42f, status = ComposerAttachmentStatus.Preparing)
                }
                val attachment = withContext(Dispatchers.IO) {
                    encodeAssistantImageAttachment(uri, attachmentId)
                }
                val existingBytes = uiState.composerAttachments
                    .filter { it.id != attachmentId && it.isReady }
                    .sumOf { it.sizeBytes?.toLong() ?: 0L }
                val nextTotalBytes = existingBytes + (attachment.sizeBytes?.toLong() ?: 0L)
                if (nextTotalBytes > ASSISTANT_IMAGE_TOTAL_MAX_BYTES) {
                    throw IOException("视觉附件总大小超过 10 MB，请移除部分图片后再试。")
                }
                updateComposerAttachment(attachmentId) {
                    it.copy(
    ...
    private fun updateComposerAttachment(
        id: String,
        block: (ComposerAttachment) -> ComposerAttachment
    ) {
        uiState = uiState.copy(
            composerAttachments = uiState.composerAttachments.map { item ->
                if (item.id == id) block(item) else item
            }
        )
    }

    private fun stripVisualAttachmentStatus(text: String): String {
        return if (text.startsWith(VISUAL_ATTACHMENT_STATUS_PREFIX)) "" else text
    }
    ...
    private fun attachmentMessageFlag(attachments: List<ChatAttachment>): String {
        ...
        return listOf("视觉附件", "图片 ${attachments.size} 张", meta)
            .distinct()
            .joinToString(" · ")
    }
    ...
    private fun encodeAssistantImageAttachment(
        uri: Uri,
        attachmentId: String = nextLocalId("image")
    ): ChatAttachment {
        val resolver = getApplication<Application>().contentResolver
        val bounds = BitmapFactory.Options().apply { inJustDecodeBounds = true }
        val canOpen = resolver.openInputStream(uri)?.use { stream ->
        ...
        val bytes = try {
            scaled.compressJpegWithinLimit(ASSISTANT_IMAGE_MAX_BYTES)
        } finally {
            scaled.recycle()
        }
        if (bytes.size > ASSISTANT_IMAGE_MAX_BYTES) {
            throw IOException("图片压缩后仍然过大，请裁剪后再试。")
        }
    ...
    private fun Bitmap.compressJpegWithinLimit(maxBytes: Int): ByteArray {
        val output = ByteArrayOutputStream()
        var quality = ASSISTANT_IMAGE_JPEG_QUALITY
        var bytes: ByteArray
        do {
            output.reset()
        ...
        } while (bytes.size > maxBytes && quality >= ASSISTANT_IMAGE_MIN_JPEG_QUALITY)
        return bytes
    }
    ...
    fun setModelCardGlassStyle(style: ModelCardGlassStyle) {
        uiState = uiState.copy(modelCardGlassStyle = style)
    }
    ...
    fun setGlassIntensity(value: Float) {
        ...
        uiState = uiState.copy(
            glassIntensity = clamped,
            glassPreset = detectPreset(clamped, uiState.motionIntensity)
        )
    ...
    fun setMotionIntensity(value: Float) {
        ...
        uiState = uiState.copy(
            ...
            glassPreset = detectPreset(uiState.glassIntensity)
        )
    ...
    private fun detectPreset(
        glass: Float,
        motion: Float = uiState.motionIntensity
    ): GlassPreset {
