...
private class OpenGlCardViewHolder {
    @Volatile var view: OpenGLGlassCardTextureView? = null
}
...
fun OpenGLGlassCardLayer(
    ...
    val viewHolder = remember { OpenGlCardViewHolder() }
    ...
    BoxWithConstraints(
        modifier = modifier.onGloballyPositioned { coordinates ->
            ...
            val sample = topLeft - (origin?.rootOffset() ?: Offset.Zero)
            val view = viewHolder.view
            if (view != null) {
                val dirty = view.setSamplingSpec(
                    sample.x,
                    sample.y,
                    backdrop.fullWidthPx.toFloat(),
                    backdrop.fullHeightPx.toFloat()
                )
                if (dirty) view.requestRender()
            }
        }
    ) {
        ...
        AndroidView(
            ...
            factory = { context ->
                OpenGLGlassCardTextureView(context).also { viewHolder.view = it }
            },
            update = { view ->
...
private class OpenGLGlassCardRenderer {
    ...
    fun setGlassSpec(w: Float, h: Float, radius: Float, intensity: Float) {
        widthPx = w.coerceAtLeast(1f)
        heightPx = h.coerceAtLeast(1f)
        radiusPx = radius
        this.intensity = intensity
    ...
    fun setSamplingSpec(x: Float, y: Float, rootW: Float, rootH: Float) {
        originX = x
        originY = y
        this.rootW = rootW.coerceAtLeast(1f)
        this.rootH = rootH.coerceAtLeast(1f)
    ...
    fun onDrawFrame() {
        ...
        GLES20.glUniform4f(rectHandle, 0f, 0f, widthPx, heightPx)
        GLES20.glUniform1f(radiusHandle, radiusPx.coerceIn(2f, max(widthPx, heightPx)))
        GLES20.glUniform1f(textureReadyHandle, if (ready) 1f else 0f)
        val currentStyle = style
        GLES20.glUniform4f(materialHandle, currentStyle.openGlVisibility.coerceIn(0f, 20f), currentStyle.openGlMaxAlpha.coerceIn(0f, 1f) * intensity, currentStyle.edgeBrightness.coerceIn(-4f, 4f), currentStyle.bodyAlpha.coerceIn(-4f, 4f))
        ...
        vertices.clear()
        vertices.put(floatArrayOf(-1f, -1f, 1f, -1f, -1f, 1f, 1f, 1f))
        vertices.position(0)
    ...
    private companion object {
        ...
        const val FRAGMENT_SHADER = """
            ...
            float edgeMask(float sd) {
                return 1.0 - smoothstep(-1.00, 2.25, sd);
            }
            ...
            void main() {
                ...
                float cornerCurve = clamp(abs(normal.x * normal.y) * 2.15, 0.0, 1.0);
                float edgeFade = smoothstep(0.16, 0.92, mask);
                float cornerSoften = 1.0 - cornerCurve * 0.34;
                float edgeWide = (1.0 - smoothstep(0.0, edgeWidth, inside)) * edgeFade;
                float edgeCore = (1.0 - smoothstep(0.0, max(edgeWidth * 0.26, 2.0), inside)) * edgeFade * cornerSoften;
                ...
                float lensMix = edgeCore * sat(max(uRefraction.z, 0.0)) * 0.24;
                vec3 color = mix(soft, lens, lensMix);

                float brightBand = edgeCore * mix(0.030, 0.016, cornerCurve);
                float darkBand = smoothstep(edgeWidth * 0.30, edgeWidth, inside) * edgeWide * 0.036;
                ...
                float alpha = clamp(uMaterial.x * uMaterial.y, 0.0, 1.0) * mask;
