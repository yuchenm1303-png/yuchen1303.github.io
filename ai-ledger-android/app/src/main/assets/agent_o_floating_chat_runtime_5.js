function handleChatAction(action,payload={}){
    ...
    if(action==='memory.open'){postChatAction(action);return;}
    if(action==='skill.open'){postChatAction(action);return;}
    if(action==='panel.collapse'){closeQuickPanels();postChatAction(action);setForm(0);return;}
    if(action==='chat.clear'){
      stopMockReply();clearTimeout(composerDispatchTimer);chatState.messages=[];chatState.isSending=false;chatState.composerText='';chatState.attachment=null;renderFloatingChat(true);postChatAction(action);showChatToast('对话已清空');return;
    }
    if(action==='chat.copy'){
      const message=chatState.messages.find(item=>item.id===payload.messageId);if(message)copyChatText(message.text||message.errorText||'');postChatAction(action,payload);return;
    }
    if(action==='chat.retry'){
      const index=chatState.messages.findIndex(item=>item.id===payload.messageId);const previous=[...chatState.messages.slice(0,index)].reverse().find(item=>item.role==='user');postChatAction(action,payload);
      if(!bridgeIsAvailable()&&previous)showChatToast('原生聊天桥未连接');return;
    }
    if(action==='chat.stop'){
      stopMockReply();chatState.isSending=false;chatState.messages=chatState.messages.map(item=>item.status==='sending'?{...item,text:item.text==='正在思考…'?'已停止生成。':item.text,status:'stopped'}:item);renderFloatingChat(true);postChatAction(action);return;
    }
    if(action==='chat.send'){
      const text=String(payload.text||chatState.composerText||'').trim();if(!text&&!chatState.attachment)return;
      ...
      else showChatToast('原生聊天桥未连接');return;
    }
  ...
  function connectFloatingChat(adapter){chatBridgeAdapter=adapter||null;chatState.bridgeConnected=bridgeIsAvailable();renderBridgeStatus();return window.GuiPlusFloatingChat;}
  ...
  window.addEventListener('message',event=>{
    const data=event.data;if(!data||typeof data!=='object'||data.source!=='gui-plus-native')return;
  ...
  shell.addEventListener('pointerdown',event=>{
    ...
    targetScale=form===0 ? P.dragPressScale : 0.985;
    if(dragging){
      if(bridgeIsAvailable())postChatAction('window.dragStart',{pointerId});
      ...
      root.dataset.dragging='true';shell.setPointerCapture(pointerId);
    }
  });
  ...
  shell.addEventListener('pointermove',event=>{
    ...
    const nativeDrag=bridgeIsAvailable();
    if(nativeDrag){
      offsetX=0;offsetY=0;offsetDirty=true;
    }else{
    ...
    if(nativeDrag)postChatAction('window.drag',{dx,dy,velocity:pointerVelocity});
  ...
  shell.addEventListener('pointerup',event=>{
    ...
    if(dragging&&bridgeIsAvailable())postChatAction('window.dragEnd',{});
    dragging=false;
    targetScale=1;
