...
function parseIndex(item){const name=text(first(item?.name,item?.indexName),'');const value=text(first(item?.price,item?.value),'');if(!name||!value||value==='--')return null;const changePercent=text(first(item?.changePercent,item?.pct),'--');return{name,value,changePercent,isRising:!changePercent.startsWith('-')}}
...
function renderIndices(){
  ...
  root.innerHTML=snapshot.indices.map(item=>`<article class="index-card ${item.isRising?'rise':'fall'}"><div class="index-head"><span class="index-name">${escapeHtml(item.name)}</span><span class="index-arrow ${toneClass(item.isRising)}">${item.isRising?'↑':'↓'}</span></div><div class="index-value">${escapeHtml(item.value)}</div><div class="index-change ${toneClass(item.isRising)}">${escapeHtml(item.changePercent)}</div></article>`).join('');
}
