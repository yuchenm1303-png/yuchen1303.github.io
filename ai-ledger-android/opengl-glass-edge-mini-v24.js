...
const APP_RAW={
  ...
  rimRefractionMix:.78,
  rimAbsorption:.42,
  rimHighlightStrength:.58,
  rimHighlightPower:2.8,
  rimStrength:.95,
  rimJoinSoftness:.72
};
...
const groups=[
  ...
  {title:'连续厚玻璃边框 Continuous Glass Rim',items:[
    ['rimWidthPx','边框截面宽度',4,18],['rimCurvature','截面曲率',.1,1.5],['rimRefractionPx','边框折射距离',0,12],['rimRefractionMix','折射混合强度',0,1],['rimAbsorption','厚度吸收',0,1],['rimHighlightStrength','方向亮面强度',0,1.2],['rimHighlightPower','亮面方向集中度',1,8],['rimStrength','边框整体强度',0,1],['rimJoinSoftness','主体衔接柔度',0,1]
  ]}
];
...
function initGl(){
  ...
  const names=['a','uRes','uOrigin','uRoot','uBlurTexture','uMat','uBodyLensA','uBodyLensB','uBody','uRimA','uRimB','uRadius','uIntensity','uRimMode','uRimJoinSoftness'];
...
function render(){
  ...
  gl.uniform1f(L.uRadius,46*d);gl.uniform1f(L.uIntensity,p.glassIntensity);gl.uniform1f(L.uRimMode,p.rimMode);gl.uniform1f(L.uRimJoinSoftness,p.rimJoinSoftness);
  ...
  gl.uniform4f(L.uRimA,p.rimWidthPx*d,p.rimCurvature,p.rimRefractionPx*d,p.rimRefractionMix);gl.uniform4f(L.uRimB,p.rimAbsorption,p.rimHighlightStrength,p.rimHighlightPower,p.rimStrength);
...
function updateUi(){groups.flatMap(g=>g.items).forEach(([k])=>{const label=$('v-'+k),input=$('i-'+k);if(label)label.textContent=fmt(p[k]);if(input&&Math.abs(Number(input.value)-p[k])>1e-9)input.value=p[k]});syncModeUi();out.textContent=JSON.stringify({mode:'v25ContinuousGlassRimSmoothJoin',rimMode:p.rimMode===0?'bodyOnly':'continuousRim',...p},null,2)}
...
document.querySelectorAll('[data-rim-mode]').forEach(btn=>btn.addEventListener('click',()=>{p.rimMode=Number(btn.dataset.rimMode);updateUi();render()}));
resetBodyBtn.addEventListener('click',()=>{p={...APP_RAW};updateUi();rebuildBackdrop();render()});
