...
const APP_RAW={
  ...
  edgeMode:1,bodyEdgeWidthPx:30,bodyEdgePullPx:84,bodyEdgeCurve:.72,bodyEdgeTangentialStretch:1.15
};
...
const groups=[
  ...
  {title:'主体折射场边缘增强 Integrated Body Edge',items:[['bodyEdgeWidthPx','主体边缘作用宽度',4,60],['bodyEdgePullPx','主体边缘向内拉力',0,160],['bodyEdgeCurve','主体边缘回落曲线',.25,3],['bodyEdgeTangentialStretch','主体边缘切向拉伸',0,2.5]]}
];
...
function prepareShellUi(){
  ...
  if(h)h.textContent='V26 · 单一主体折射场';
  if(s)s.textContent='single V25.3 body field · integrated edge deformation';
  if(t)t.textContent='主体折射场边缘对比 Body Edge Comparison';
  ...
  document.querySelectorAll('[data-edge-mode]').forEach(btn=>{btn.textContent=btn.dataset.edgeMode==='0'?'原始 V25.3 主体':'主体边缘增强（同一折射场）'});
  bgUpload.type='file';bgUpload.accept='image/*';
}
prepareShellUi();
function initGl(){
  gl=cv.getContext('webgl',{alpha:true,premultipliedAlpha:false});if(!gl)throw new Error('当前浏览器未启用 WebGL');
...
function drawShiftAverage(dstCtx,src,w,h,step,horizontal){configureSmoothContext(dstCtx);dstCtx.clearRect(0,0,w,h);dstCtx.save();dstCtx.globalCompositeOperation='lighter';dstCtx.globalAlpha=.2;for(let i=-2;i<=2;i++){dstCtx.drawImage(src,horizontal?i*step:0,horizontal?0:i*step,w,h)}dstCtx.restore();dstCtx.save();dstCtx.globalCompositeOperation='destination-over';dstCtx.globalAlpha=1;dstCtx.drawImage(src,0,0,w,h);dstCtx.restore()}
function applyFullResolutionBlur(source,w,h,blurPx){configureSmoothContext(blurCtx);blurCtx.clearRect(0,0,w,h);if(blurPx<=.025){blurCtx.drawImage(source,0,0,w,h);return}resizeCanvas(blurLevelA,w,h);resizeCanvas(blurLevelB,w,h);const passCount=Math.max(1,Math.min(3,Math.ceil(p.iterations/4))),step=Math.max(.25,blurPx/Math.sqrt(2*passCount));let current=source;for(let pass=0;pass<passCount;pass++){drawShiftAverage(blurLevelACtx,current,w,h,step,true);drawShiftAverage(blurLevelBCtx,blurLevelA,w,h,step,false);current=blurLevelB}blurCtx.drawImage(current,0,0,w,h)}
...
function rebuildBackdrop(){const d=stageDpr(),r=stage.getBoundingClientRect(),w=Math.max(1,Math.round(r.width*d)),h=Math.max(1,Math.round(r.height*d));resizeCanvas(bg,w,h);resizeCanvas(sourceCanvas,w,h);resizeCanvas(colorCanvas,w,h);resizeCanvas(blurCanvas,w,h);drawBackdrop(ctx,w,h,true);drawBackdrop(sourceCtx,w,h,false);configureSmoothContext(colorCtx);colorCtx.clearRect(0,0,w,h);colorCtx.save();colorCtx.filter=`brightness(${p.brightness}) contrast(${p.contrast}) saturate(${p.saturation})`;colorCtx.drawImage(sourceCanvas,0,0,w,h);colorCtx.filter='none';colorCtx.restore();applyFullResolutionBlur(colorCanvas,w,h,effectiveBlurPx(d));drawGlassBackdrop();uploadBackdropTexture();backdropRevision++}
...
function render(){drawGlassBackdrop();gl.clearColor(0,0,0,0);gl.clear(gl.COLOR_BUFFER_BIT);gl.useProgram(program);gl.bindBuffer(gl.ARRAY_BUFFER,buffer);gl.enableVertexAttribArray(L.a);gl.vertexAttribPointer(L.a,2,gl.FLOAT,false,0,0);const d=stageDpr(),q=glassRect();gl.uniform2f(L.uRes,cv.width,cv.height);gl.uniform2f(L.uOrigin,q.x*d,q.y*d);gl.uniform2f(L.uRoot,bg.width,bg.height);gl.uniform1f(L.uRadius,46*d);gl.uniform1f(L.uIntensity,p.glassIntensity);gl.uniform1f(L.uEdgeMode,p.edgeMode);gl.uniform4f(L.uMat,p.bodyVisibility,p.bodyMaxAlpha,p.bodyOutputBrightness,0);gl.uniform4f(L.uBodyLensA,p.bodyLensBasePull*d,p.bodyLensPullDp*d,p.bodyLensConcentration,p.bodyLensCornerBoost);gl.uniform4f(L.uBodyLensB,p.bodyLensExtraDistance*d,p.bodyLensReachDp*d,p.bodyLensDark,p.bodyLensDebug);gl.uniform4f(L.uBody,p.bodyLowFrequencyWidth,p.bodyLowFrequencyCurve,p.bodyLowFrequencyGain,p.bodyBrightness);gl.uniform4f(L.uBodyEdge,p.bodyEdgeWidthPx*d,p.bodyEdgePullPx*d,p.bodyEdgeCurve,p.bodyEdgeTangentialStretch);gl.activeTexture(gl.TEXTURE0);gl.bindTexture(gl.TEXTURE_2D,blurTex);gl.uniform1i(L.uBlurTexture,0);gl.drawArrays(gl.TRIANGLE_STRIP,0,4)}
