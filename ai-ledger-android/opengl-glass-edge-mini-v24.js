...
const groups=[
  ...
  {title:'统一折射厚玻璃边框 Unified Glass Rim',items:[
    ['rimWidthPx','边框截面宽度',4,18],['rimCurvature','截面曲率',.1,1.5],['rimRefractionPx','边框折射距离',0,12],['rimRefractionMix','边框流场强度',0,1],['rimJoinWidth','两端接入宽度',.05,.45],['rimAbsorption','厚度吸收',0,1],['rimHighlightStrength','方向亮面强度',0,1.2],['rimHighlightPower','亮面方向集中度',1,8],['rimStrength','边框整体强度',0,1]
  ]}
];
...
function initGl(){
  ...
  const names=['a','uRes','uOrigin','uRoot','uBlurTexture','uMat','uBodyLensA','uBodyLensB','uBody','uRimA','uRimB','uRadius','uIntensity','uRimMode','uRimJoinWidth'];
...
function render(){
  ...
  gl.uniform1f(L.uRadius,46*d);gl.uniform1f(L.uIntensity,p.glassIntensity);gl.uniform1f(L.uRimMode,p.rimMode);gl.uniform1f(L.uRimJoinWidth,p.rimJoinWidth);
...
function updateUi(){groups.flatMap(g=>g.items).forEach(([k])=>{const label=$('v-'+k),input=$('i-'+k);if(label)label.textContent=fmt(p[k]);if(input&&Math.abs(Number(input.value)-p[k])>1e-9)input.value=p[k]});syncModeUi();out.textContent=JSON.stringify({mode:'v25UnifiedOpticalFlowRim',rimMode:p.rimMode===0?'bodyOnly':'unifiedRimFlow',...p},null,2)}
