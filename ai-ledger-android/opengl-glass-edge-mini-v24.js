...
const groups=[
  ...
  {title:'连续厚玻璃边框 Continuous Glass Rim',items:[
    ['rimWidthPx','边框截面宽度',4,18],['rimCurvature','截面曲率',.1,1.5],['rimRefractionPx','边框折射距离',0,12],['rimRefractionMix','折射混合强度',0,1],['rimAbsorption','厚度吸收',0,1],['rimHighlightStrength','方向亮面强度',0,1.2],['rimHighlightPower','亮面方向集中度',1,8],['rimStrength','边框整体强度',0,1]
  ]}
];
...
function updateUi(){groups.flatMap(g=>g.items).forEach(([k])=>{const label=$('v-'+k),input=$('i-'+k);if(label)label.textContent=fmt(p[k]);if(input&&Math.abs(Number(input.value)-p[k])>1e-9)input.value=p[k]});syncModeUi();out.textContent=JSON.stringify({mode:'v25ContinuousGlassRim',rimMode:p.rimMode===0?'bodyOnly':'continuousRim',...p},null,2)}
