...
const APP_RAW={
  ...
  innerStartDp:3.2,
  innerWidthDp:20,
  innerTangentSpreadDp:11.5,
  innerTangentStrength:.88,
  innerNormalPullDp:5.5,
  innerCornerBoost:.52,
  innerColorGain:1.08,
  innerAlpha:.86,
  ...
  rimCoreWidthDp:5.8,
  rimOuterWidthDp:1.8,
  rimInnerWidthDp:11.5,
  rimTransmission:.82,
  rimTintStrength:.34,
  rimInnerAbsorption:.52,
  rimOuterBrightness:.46,
  rimCornerBoost:.38,
  rimMaterialAlpha:.72,
  rimClearMix:.36,

  specularWidthDp:1.35,
  specularIntensity:.92,
  specularLightX:-.72,
  specularLightY:-.68,
  specularFresnel:1.7,
  specularCornerBoost:.48,
  specularTemperature:-.12,
  specularAlpha:.88
};
const LEGACY_FIXED={
...
const groups=[
  ...
  {title:'新边缘 · Inner Refraction Band',items:[
    ['innerStartDp','内侧起始距离',0,10],['innerWidthDp','内侧带宽度',4,36],['innerTangentSpreadDp','切线扩散距离',0,32],['innerTangentStrength','切线扩散强度',0,1],['innerNormalPullDp','法线拉入量',0,18],['innerCornerBoost','圆角扩散增强',0,2],['innerColorGain','内容颜色增益',.4,1.8],['innerAlpha','内侧带透明度',0,1]
  ]},
  {title:'新边缘 · Rim Material Band',items:[
    ['rimCoreWidthDp','实体边核心宽度',.5,14],['rimOuterWidthDp','外侧宽度',.25,3],['rimInnerWidthDp','内侧宽度',3,20],['rimTransmission','透射率',0,1.4],['rimTintStrength','背景染色强度',0,1.5],['rimInnerAbsorption','内沿吸收',0,2],['rimOuterBrightness','外沿亮度',0,2],['rimCornerBoost','圆角厚度增强',0,2],['rimMaterialAlpha','材质透明度',0,1],['rimClearMix','清晰背景混合',0,1]
  ]},
  {title:'新边缘 · Specular Rim',items:[
    ['specularWidthDp','高光宽度',.2,3],['specularIntensity','高光强度',0,3],['specularLightX','光源方向 X',-1,1],['specularLightY','光源方向 Y',-1,1],['specularFresnel','Fresnel 曲线',.2,5],['specularCornerBoost','圆角高光增强',0,3],['specularTemperature','高光色温',-1,1],['specularAlpha','高光透明度',0,1]
  ]},
...
function initGl(){
  ...
  const names=['a','uRes','uOrigin','uRoot','uShapeOffset','uShapeSize','uBlurTexture','uLensTexture','uMat','uBodyLensA','uBodyLensB','uBody','uLegacyMaterial','uLegacyRefraction','uLegacyOptics','uNewInnerA','uNewInnerB','uNewRimA','uNewRimB','uNewRimC','uNewSpecA','uNewSpecB','uMode','uRadius','uIntensity','uTextureReady'];
...
function render(){
  ...
  const d=stageDpr(),q=glassRect(),overscan=3*d;
  gl.uniform2f(L.uRes,cv.width,cv.height);gl.uniform2f(L.uOrigin,q.x*d,q.y*d);gl.uniform2f(L.uRoot,bg.width,bg.height);gl.uniform2f(L.uShapeOffset,overscan,overscan);gl.uniform2f(L.uShapeSize,q.w*d,q.h*d);
  ...
  gl.uniform4f(L.uNewInnerA,p.innerStartDp*d,p.innerWidthDp*d,p.innerTangentSpreadDp*d,p.innerTangentStrength);gl.uniform4f(L.uNewInnerB,p.innerNormalPullDp*d,p.innerCornerBoost,p.innerColorGain,p.innerAlpha);
  gl.uniform4f(L.uNewRimA,p.rimCoreWidthDp*d,p.rimOuterWidthDp*d,p.rimInnerWidthDp*d,p.rimTransmission);gl.uniform4f(L.uNewRimB,p.rimTintStrength,p.rimInnerAbsorption,p.rimOuterBrightness,p.rimCornerBoost);gl.uniform4f(L.uNewRimC,p.rimMaterialAlpha,p.rimClearMix,0,0);
  gl.uniform4f(L.uNewSpecA,p.specularWidthDp*d,p.specularIntensity,p.specularLightX,p.specularLightY);gl.uniform4f(L.uNewSpecB,p.specularFresnel,p.specularCornerBoost,p.specularTemperature,p.specularAlpha);gl.uniform4f(L.uMode,p.edgeMode,p.debugView,p.compareSplit,0);
...
function syncModeUi(){document.querySelectorAll('[data-edge-mode]').forEach(btn=>btn.classList.toggle('on',Number(btn.dataset.edgeMode)===p.edgeMode));debugView.value=String(p.debugView);legacyDomRim.classList.toggle('off',p.edgeMode===1||p.debugView>0);legacyDomRim.classList.toggle('compare',p.edgeMode===2&&p.debugView===0);legacyDomRim.style.setProperty('--legacy-split',`${p.compareSplit*100}%`)}
function updateUi(){groups.flatMap(g=>g.items).forEach(([k])=>{const label=$('v-'+k),input=$('i-'+k);if(label)label.textContent=fmt(p[k]);if(input&&Math.abs(Number(input.value)-p[k])>1e-9)input.value=p[k]});syncModeUi();out.textContent=JSON.stringify({mode:'v25BodyWithThreeLayerEdge',edgeMode:p.edgeMode===0?'legacy':p.edgeMode===1?'new':'compare',debugView:p.debugView,legacySourceCommit:'9a6e4ac7605da3859ff9accd4a33fe0bab7a9ddc',...p},null,2)}
...
document.querySelectorAll('[data-preset]').forEach(btn=>btn.addEventListener('click',()=>{const edgeMode=p.edgeMode;p={...APP_RAW,edgeMode};if(btn.dataset.preset==='legacyFixed')Object.assign(p,LEGACY_FIXED,{edgeMode:0});if(btn.dataset.preset==='strong')Object.assign(p,{innerWidthDp:25,innerTangentSpreadDp:19,innerTangentStrength:1,innerNormalPullDp:9,innerColorGain:1.16,innerAlpha:1,rimInnerWidthDp:14,rimMaterialAlpha:.84,rimTintStrength:.58,rimOuterBrightness:.72,specularIntensity:1.45,specularCornerBoost:.82});updateUi();rebuildBackdrop();render()}));
...
try{initGl();buildControls();requestAnimationFrame(()=>{resize();scroll.scrollTop=500;render()})}catch(err){errorEl.textContent=String(err&&err.stack||err);console.error(err)}
