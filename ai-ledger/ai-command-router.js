(() => {
  ...
  const ROUTER_FLAG = '__aiCommandRouterInstalled';
  ...
  function parseNavigationStart(text) {
    ...
    return {
      kind: resolved.placeAddressMissing ? 'need_user_info' : 'local_command',
      intent: 'navigation.start',
      confidence: 0.95,
      source: 'local_rule',
      missing: resolved.placeAddressMissing ? `${resolved.matchedPlaceLabel || resolved.alias}地址` : '',
      command: {
        id: createId('nav'),
        type: 'navigate',
        title: `${appName}导航`,
  ...
  function routeText(text) {
    ...
    const local = parseNavigationPreference(raw) || parseNavigationStart(raw);
  ...
  function patchAssistantPreferencesApi() {
    ...
    if (!api || api.__commandRouterPatched) return;
    api.__commandRouterPatched = true;
    ...
    api.resolveDestination = (destination) => {
      ...
      return {
        ...
        matchedPlace: resolved.matchedPlaceKey ? {
          key: resolved.matchedPlaceKey,
          label: resolved.matchedPlaceLabel,
          address: resolved.placeAddressMissing ? '' : resolved.destination,
        } : null,
  ...
  function patchMobileActions() {
    ...
    if (!actions || actions.__commandRouterPatched) return;
    actions.__commandRouterPatched = true;
  ...
  function installSubmitRouter() {
    ...
    if (!form || !input || form.dataset.commandRouterInstalled === 'true') return;
    form.dataset.commandRouterInstalled = 'true';
