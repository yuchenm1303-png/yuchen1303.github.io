(() => {
  ...
  const I18N = {
    "zh-CN": {
      ...
      panelTitle: "显示与语言",
      panelDesc: "调整语言、字体、玻璃质感和动效。透明度 0 为默认液态玻璃样式，向左更通透，向右更厚实。",
      language: "语言",
      fontSize: "字体大小",
    ...
    en: {
      ...
      panelDesc: "Adjust language, font size, glass effect and motion. Opacity 0 keeps the default liquid glass look; negative is clearer, positive is thicker.",
      language: "Language",
      fontSize: "Font size",
      ...
      reset: "Reset",
      preview: "Preview: 0 is the default liquid glass effect. Negative is clearer, positive is whiter and thicker.",
    ...
    ja: {
      ...
      panelTitle: "表示と言語",
      panelDesc: "言語、文字サイズ、ガラス効果、アニメーションを調整します。透明度 0 は標準の液体ガラス効果です。",
      language: "言語",
      fontSize: "文字サイズ",
      ...
      save: "表示設定を保存",
      reset: "初期設定に戻す",
      preview: "プレビュー：0 が標準の液体ガラス効果です。負の値はより透明、正の値はより白く厚くなります。",
      saved: "表示設定を保存しました",
  ...
  function normalizeSettings(settings) {
    return {
      ...
      glassOpacity: clamp(Number(settings.glassOpacity), -50, 50, DEFAULTS.glassOpacity),
  ...
  function alpha(base, offset) {
    return Math.max(0.010, Math.min(0.115, base + offset * 0.00115)).toFixed(3);
  }
  ...
  function installStyle() {
    ...
    style.textContent = `
      ...
      .glass-card,.chat-shell,.summary-card,.metric-card,.chart-card,.tool-card,.auth-sheet,.mobile-command-card{
        ...
        backdrop-filter:blur(var(--assistant-glass-blur)) saturate(1.18) brightness(1.05) contrast(1.02) !important;
        -webkit-backdrop-filter:blur(var(--assistant-glass-blur)) saturate(1.18) brightness(1.05) contrast(1.02) !important;
      }
      ...
      .appearance-plus-desc{margin:0;color:rgba(214,224,246,.66);font-size:13px;line-height:1.6}
      .appearance-plus-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}
      .appearance-plus-field{display:grid;gap:7px}
      .appearance-plus-field span{font-size:13px;font-weight:850;color:rgba(230,238,255,.78)}
      .appearance-plus-field select{width:100%;border:1px solid rgba(255,255,255,.16);border-radius:16px;padding:12px 13px;background:rgba(255,255,255,var(--assistant-glass-control-alpha));color:rgba(248,250,255,.96);font:inherit;outline:none}
      .appearance-plus-field input[type="range"]{width:100%;accent-color:#73e7ff}
      .appearance-range-row{display:flex;align-items:center;gap:12px;padding:10px 12px;border-radius:16px;background:rgba(255,255,255,var(--assistant-glass-control-alpha));border:1px solid rgba(255,255,255,.14)}
      .appearance-range-value{min-width:54px;text-align:right;color:rgba(248,250,255,.96);font-weight:850;font-size:13px}
      .appearance-toggle-row{display:flex;gap:10px;flex-wrap:wrap}
      .appearance-toggle{border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,var(--assistant-glass-control-alpha));color:rgba(230,238,255,.78);border-radius:999px;padding:10px 13px;font-weight:850;cursor:pointer}
      ...
      .appearance-preview{padding:14px;border-radius:20px;border:1px solid rgba(255,255,255,.16);background:rgba(255,255,255,var(--assistant-glass-preview-alpha));backdrop-filter:blur(var(--assistant-glass-blur));-webkit-backdrop-filter:blur(var(--assistant-glass-blur));color:rgba(214,224,246,.72);line-height:1.6}
  ...
  function installPanel() {
    ...
    panel.innerHTML = `
      ...
      <div class="appearance-plus-grid">
        <label class="appearance-plus-field"><span data-i18n="language">${escapeHtml(t("language"))}</span><select id="appearanceLanguageSelect"><option value="zh-CN">简体中文</option><option value="en">English</option><option value="ja">日本語</option></select></label>
        <label class="appearance-plus-field"><span data-i18n="fontSize">${escapeHtml(t("fontSize"))}</span><select id="appearanceFontSelect"><option value="small">${escapeHtml(t("small"))}</option><option value="normal">${escapeHtml(t("normal"))}</option><option value="large">${escapeHtml(t("large"))}</option><option value="xlarge">${escapeHtml(t("xlarge"))}</option></select></label>
        <label class="appearance-plus-field"><span data-i18n="glassOpacity">${escapeHtml(t("glassOpacity"))}</span><div class="appearance-range-row"><input id="appearanceGlassOpacity" type="range" min="-50" max="50" step="1" /><strong class="appearance-range-value" data-range-value="appearanceGlassOpacity"></strong></div></label>
        <label class="appearance-plus-field"><span data-i18n="glassBlur">${escapeHtml(t("glassBlur"))}</span><div class="appearance-range-row"><input id="appearanceGlassBlur" type="range" min="4" max="32" step="1" /><strong class="appearance-range-value" data-range-value="appearanceGlassBlur"></strong></div></label>
  ...
  function bindPanelEvents() {
    ...
    document.querySelectorAll("[data-appearance-toggle]").forEach((button) => {
      button.addEventListener("click", () => { setToggleGroup(button.dataset.appearanceToggle, button.dataset.value); applySettings(currentFormSettings()); });
    });
