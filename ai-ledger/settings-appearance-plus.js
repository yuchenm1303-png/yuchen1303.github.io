(() => {
  ...
  const I18N = {
    "zh-CN": {
      appTitle: "AI助手", aiEyebrow: "AI多功能助手", aiTitle: "对话", aiSubtext: "聊天、记账、提醒与任务", aiPlaceholder: "和我说点什么",
      navAi: "AI助手", navTools: "功能", navSettings: "设置", settingsEyebrow: "设置中心", settingsTitle: "应用设置", settingsSubtext: "接入登录、云端同步与个性化外观",
      panelTitle: "显示与语言", panelDesc: "调整语言、字体、玻璃质感和动效。透明度 0 为默认液态玻璃样式，向左更通透，向右更厚实。",
      language: "语言", fontSize: "字体大小", glassOpacity: "玻璃透明度偏移", glassBlur: "玻璃模糊强度", motion: "动画效果", compact: "紧凑模式",
      save: "保存显示设置", reset: "恢复默认", preview: "预览：0 就是默认液态玻璃效果。负值更透明，正值更偏白、更厚。", saved: "已保存显示设置", resetDone: "已恢复默认显示设置",
      on: "开启", off: "关闭", small: "小", normal: "标准", large: "大", xlarge: "超大",
    },
    en: {
      appTitle: "AI Assistant", aiEyebrow: "AI multifunction assistant", aiTitle: "Chat", aiSubtext: "Chat, ledger, reminders and tasks", aiPlaceholder: "Say something",
      navAi: "Assistant", navTools: "Tools", navSettings: "Settings", settingsEyebrow: "Settings", settingsTitle: "App Settings", settingsSubtext: "Account, sync and personalized appearance",
      panelTitle: "Display & Language", panelDesc: "Opacity 0 keeps the default liquid glass look; negative is clearer, positive is thicker.",
      language: "Language", fontSize: "Font size", glassOpacity: "Glass opacity offset", glassBlur: "Glass blur", motion: "Motion", compact: "Compact mode",
      save: "Save display settings", reset: "Reset", preview: "Preview: 0 is the default liquid glass effect. Negative is clearer, positive is whiter and thicker.", saved: "Display settings saved", resetDone: "Display settings reset",
      on: "On", off: "Off", small: "Small", normal: "Normal", large: "Large", xlarge: "Extra large",
    },
    ja: {
      appTitle: "AIアシスタント", aiEyebrow: "AI多機能アシスタント", aiTitle: "チャット", aiSubtext: "会話・家計簿・リマインダー・タスク", aiPlaceholder: "何か話してください",
      navAi: "AI助手", navTools: "機能", navSettings: "設定", settingsEyebrow: "設定センター", settingsTitle: "アプリ設定", settingsSubtext: "ログイン、同期、外観を設定します",
      panelTitle: "表示と言語", panelDesc: "透明度 0 は標準の液体ガラス効果です。負の値はより透明、正の値はより白く厚くなります。",
      language: "言語", fontSize: "文字サイズ", glassOpacity: "ガラス透明度オフセット", glassBlur: "ガラスぼかし", motion: "アニメーション", compact: "コンパクト表示",
      save: "表示設定を保存", reset: "初期設定に戻す", preview: "プレビュー：0 が標準の液体ガラス効果です。", saved: "表示設定を保存しました", resetDone: "表示設定を初期化しました",
      on: "オン", off: "オフ", small: "小", normal: "標準", large: "大", xlarge: "特大",
    },
  };
  ...
  function installStyle() {
    ...
    style.textContent = `
      ...
      body.assistant-motion-off .reveal,
      body.assistant-motion-off .view.active .reveal,
      body.assistant-motion-off .appearance-plus-card{
        opacity:1 !important;
  ...
  function refreshPanelText(settings = readSettings()) {
    ...
    if (preview) preview.innerHTML = `<strong>${escapeHtml(t("panelTitle"))}</strong> · ${escapeHtml(t("preview"))}`;
  ...
  function installPanel() {
    ...
    panel.innerHTML = `
      ...
      <div class="appearance-plus-grid">
        <label class="appearance-plus-field"><span data-i18n="language">${escapeHtml(t("language"))}</span><div class="appearance-select-wrap"><select id="appearanceLanguageSelect"><option value="zh-CN">简体中文</option><option value="en">English</option><option value="ja">日本語</option></select><b>⌄</b></div></label>
        <label class="appearance-plus-field"><span data-i18n="fontSize">${escapeHtml(t("fontSize"))}</span><div class="appearance-select-wrap"><select id="appearanceFontSelect"><option value="small">${escapeHtml(t("small"))}</option><option value="normal">${escapeHtml(t("normal"))}</option><option value="large">${escapeHtml(t("large"))}</option><option value="xlarge">${escapeHtml(t("xlarge"))}</option></select><b>⌄</b></div></label>
        <label class="appearance-plus-field"><span data-i18n="glassOpacity">${escapeHtml(t("glassOpacity"))}</span><div class="appearance-range-row"><input id="appearanceGlassOpacity" type="range" min="-100" max="100" step="1" /><strong class="appearance-range-value" data-range-value="appearanceGlassOpacity"></strong></div></label>
        <label class="appearance-plus-field"><span data-i18n="glassBlur">${escapeHtml(t("glassBlur"))}</span><div class="appearance-range-row"><input id="appearanceGlassBlur" type="range" min="0" max="30" step="1" /><strong class="appearance-range-value" data-range-value="appearanceGlassBlur"></strong></div></label>
        <div class="appearance-plus-field"><span data-i18n="motion">${escapeHtml(t("motion"))}</span><div class="appearance-toggle-row"><button class="appearance-toggle" type="button" data-appearance-toggle="motion" data-value="on"><span data-toggle-label="on">${escapeHtml(t("on"))}</span></button><button class="appearance-toggle" type="button" data-appearance-toggle="motion" data-value="off"><span data-toggle-label="off">${escapeHtml(t("off"))}</span></button></div></div>
        <div class="appearance-plus-field"><span data-i18n="compact">${escapeHtml(t("compact"))}</span><div class="appearance-toggle-row"><button class="appearance-toggle" type="button" data-appearance-toggle="compact" data-value="off"><span data-toggle-label="off">${escapeHtml(t("off"))}</span></button><button class="appearance-toggle" type="button" data-appearance-toggle="compact" data-value="on"><span data-toggle-label="on">${escapeHtml(t("on"))}</span></button></div></div>
      </div>
      <div class="appearance-preview" data-i18n="preview"><strong>${escapeHtml(t("panelTitle"))}</strong> · ${escapeHtml(t("preview"))}</div>
      <div class="settings-actions inline-actions"><button id="saveAppearancePlusBtn" class="ghost-btn" type="button" data-i18n="save">${escapeHtml(t("save"))}</button><button id="resetAppearancePlusBtn" class="ghost-btn" type="button" data-i18n="reset">${escapeHtml(t("reset"))}</button></div>
    `;
  ...
  function bindPanelEvents() {
    document.querySelector("#appearanceLanguageSelect")?.addEventListener("change", () => { const next = currentFormSettings(); applySettings(next); refreshPanelText(next); });
    document.querySelector("#appearanceFontSelect")?.addEventListener("change", () => applySettings(currentFormSettings()));
    document.querySelector("#appearanceGlassOpacity")?.addEventListener("input", (event) => { setRangeValue("appearanceGlassOpacity", event.target.value); applySettings(currentFormSettings()); });
    document.querySelector("#appearanceGlassBlur")?.addEventListener("input", (event) => { setRangeValue("appearanceGlassBlur", event.target.value, "px"); applySettings(currentFormSettings()); });
    document.querySelectorAll("[data-appearance-toggle]").forEach((button) => button.addEventListener("click", () => { setToggleGroup(button.dataset.appearanceToggle, button.dataset.value); applySettings(currentFormSettings()); }));
    document.querySelector("#saveAppearancePlusBtn")?.addEventListener("click", () => { const saved = saveSettings(currentFormSettings()); notify(getText("saved", saved)); });
    document.querySelector("#resetAppearancePlusBtn")?.addEventListener("click", () => { const saved = saveSettings({ ...DEFAULTS }); notify(getText("resetDone", saved)); });
  }

  window.AppearancePlusSettings = { read: readSettings, save: saveSettings, apply: applySettings, getText, defaults: DEFAULTS };
  installStyle();
